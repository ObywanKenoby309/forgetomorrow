// pages/api/anvil/onboarding-growth/generate.js
...
function coercePivotToMultiline(pivotLine) {
  ...
  const title = s.slice(0, idxWhy).trim();
  const rest = s.slice(idxWhy).trim();
...
`.trim();
    ...
    try {
      ...
      console.error('[anvil/onboarding-growth/generate] Responses API failed:', e?.message || e);
    }
    ...
    if (!parsed) {
      try {
        ...
        console.error('[anvil/onboarding-growth/generate] Chat Completions failed:', e?.message || e);
        return res.status(500).json({ error: 'Failed to generate plan' });
      }
    }

    if (!parsed || typeof parsed !== 'object') {
      console.error('[anvil/onboarding-growth/generate] Could not parse JSON. Raw:', rawText);
      return res.status(500).json({ error: 'Failed to generate plan' });
    }
    ...
    return res.status(200).json({ planId, roadmapId: planId, plan: parsed });
  } catch (err) {
    console.error('[anvil/onboarding-growth/generate] Unhandled error', err);
    return res.status(500).json({ error: 'Failed to generate plan' });
  }
}
