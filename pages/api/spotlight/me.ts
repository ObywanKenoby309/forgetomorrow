...
//
// New fields: hook (String?) and whyICoach (String?)
...
export default async function handler(
  ...
  if (req.method === 'POST') {
    ...
    if (existing) {
      return res.status(409).json({ error: 'Spotlight already exists' });
    }

    const { name, headline, summary, specialties, rate, availability, hook, whyICoach } =
      req.body || {};
  ...
  if (req.method === 'DELETE') {
    await prisma.hearthSpotlight.delete({ where: { userId } });
