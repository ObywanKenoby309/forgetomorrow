...
export default async function handler(req, res) {
  ...
  // PATCH = update existing pinned job (title, company, location, url)
  if (req.method === "PATCH") {
    try {
      const { pinnedId, title, company, location, url } = req.body;
      if (!pinnedId) {
        return res.status(400).json({ error: "pinnedId required" });
      }
      if (!title || !company) {
        return res.status(400).json({ error: "title and company required" });
      }

      const updated = await prisma.pinnedJob.update({
        where: { id: Number(pinnedId), userId },
        data: {
          title,
          company,
          location: location || "",
      ...
      return res.status(200).json({ success: true, pinned: updated });
    } catch (err) {
      console.error("[api/seeker/pinned-jobs] patch error:", err);
      if (err?.code === "P2025") {
        return res.status(404).json({ error: "Pinned job not found" });
      }
      return res.status(500).json({ error: "Failed to update pinned job" });
    }
  }
