// pages/anvil/onboarding-growth/results.js
...
export default function OnboardingGrowthResultsPage() {
  ...
  // ✅ accept both planId and legacy roadmapId
  const planId =
    String(router.query.planId || '').trim() ||
    String(router.query.roadmapId || '').trim();

  const [loading, setLoading] = useState(true);
  const [plan, setPlan] = useState(null);
  const [meta, setMeta] = useState(null); // createdAt, id, etc.
  ...
  const backToSelectionHref = useMemo(() => {
    ...
    return `/anvil/onboarding-growth${params.toString() ? `?${params.toString()}` : ''}`;
  }, [chrome]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // ✅ CANONICAL ENDPOINT
        const res = await fetch(`/api/anvil/onboarding-growth/get?${qs.toString()}`, {
          method: 'GET',
        });
        ...
        // Canonical returns { plan, roadmap, planId, roadmapId }
        const record = data?.plan || data?.roadmap || null;
        const loadedPlan = record?.data || null;

        setMeta(record ? { id: record.id, createdAt: record.createdAt } : null);
        setPlan(loadedPlan);
      } catch (e) {
        if (!active) return;
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
            {!planId ? (
              <div className="text-center py-10">
                <div className="text-gray-800 font-semibold">Missing planId.</div>
                <button
                  onClick={goBack}
              ...
              <div className="text-center py-10 text-gray-700">Loading your saved plan...</div>
            ) : error ? (
              ...
              <div className="text-center py-10 text-gray-700">
                No plan found for this id. Please generate a new one.
              </div>
            ) : (
