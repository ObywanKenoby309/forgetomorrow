...
function Card({ children, style }) {
  return (
    <section style={{
      ...
      padding: 16, boxShadow: '0 2px 6px rgba(0,0,0,0.06)', ...style
    }}>
      {children}
    </section>
  );
...
function Primary({ onClick, children, disabled }) {
  return (
    <button type="button" onClick={onClick} disabled={disabled}
      style={{
        background: '#FF7043', color: 'white', border: '1px solid rgba(0,0,0,0.06)',
...
export default function ApplyAssistant() {
  ...
  async function onGenerate() {
    if (!jobText.trim()) { alert('Please paste the job description.'); return; }
    setBusy(true);
    try {
      ...
      const { resumeId, coverId } = await matchTemplate({ jobText, profile: { skills: resumeDraft.skills || [] } });

      // 3) generate drafts
      const resumeOut = await writeResume({ jobText, resume: resumeDraft, style: resumeId });
      const coverOut  = await writeCover({ jobText, resume: {
        fullName: resumeDraft?.formData?.fullName || '',
        contact: [resumeDraft?.formData?.email, resumeDraft?.formData?.phone, resumeDraft?.formData?.location].filter(Boolean).join(' · '),
      ...
      try {
        ...
        localStorage.setItem('ft_resume_seed', JSON.stringify({ ...resumeOut.seed, templateId: resumeId }));
        localStorage.setItem('ft_cover_draft', JSON.stringify({ fields: coverOut.fields }));
      } catch {}
  ...
  const Header = (
    <Card style={{ textAlign: 'center' }}>
      <h1 style={{ color: '#FF7043', fontSize: 28, fontWeight: 800, margin: 0 }}>Apply Assistant</h1>
      <p style={{ color: '#546E7A', margin: '8px 0 0' }}>
        One flow → AI drafts a resume and cover letter from your job description and resume.
      </p>
    </Card>
  );
    ...
    <div style={{ display: 'grid', gap: 12 }}>
      <Card>
        ...
        <ul style={{ margin: 0, paddingLeft: 18, color: '#607D8B', fontSize: 14, display: 'grid', gap: 6 }}>
          ...
          <li>“Use my current builder draft” pulls from your last saved resume here.</li>
        </ul>
      </Card>
    </div>
  );
  ...
  return (
    ...
    >
      <div style={{ display: 'grid', gap: 16, maxWidth: 960, margin: '0 auto' }}>
        <Card>
          <div style={{ fontWeight: 800, color: '#37474F', marginBottom: 8 }}>1) Job Description</div>
          <textarea
            placeholder="Paste the job description here…"
            value={jobText}
            onChange={(e) => setJobText(e.target.value)}
            style={{ width: '100%', minHeight: 220, border: '1px solid #E0E0E0', borderRadius: 10, padding: 12, outline: 'none' }}
          />
        ...
        <Card>
          <div style={{ fontWeight: 800, color: '#37474F', marginBottom: 8 }}>2) Resume Source</div>
          <div style={{ display: 'grid', gap: 10 }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <input type="radio" name="src" checked={source === 'draft'} onChange={() => setSource('draft')} />
              <span>Use my current builder draft (recommended)</span>
            </label>
            <label style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <input type="radio" name="src" checked={source === 'blank'} onChange={() => setSource('blank')} />
              <span>Start from blank (we’ll create a minimal resume)</span>
            </label>
          </div>
          <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
            <Primary onClick={onGenerate} disabled={busy}>{busy ? 'Generating…' : 'Generate drafts & open resume'}</Primary>
            <Ghost onClick={() => window.open('/resume/create', '_blank')}>Open builder</Ghost>
          </div>
        </Card>
      </div>
    </SeekerLayout>
