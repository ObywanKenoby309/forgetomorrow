...
export default function CoachingFeedbackDashboardPage() {
  ...
  const mine = useMemo(
    () => all.filter((f) => f.coachId === coachId),
    [all, coachId]
  );
  ...
  const stats = useMemo(() => {
    ...
    const avg =
      mine.reduce((sum, r) => sum + Number(r.rating || 0), 0) / mine.length;
  ...
  return (
    <CoachingLayout
      ...
      activeNav="feedback"
      headerDescription="Track ratings and testimonials collected from your public feedback form."
        ...
        <section style={section}>
          ...
          >
            ...
            <a
              ...
              title="Open public feedback form for this Coach ID"
            >
              Open Public Form →
            </a>
          </div>
          ...
          >
            <input
              ...
              placeholder="Coach ID (e.g., your-handle or demo-coach)"
              style={input}
            />
            <select
              value={ratingFilter}
        ...
        <section style={section}>
          ...
          >
            <KPI
              ...
              value={stats.count ? `${stats.avg} / 5` : '—'}
            />
            <KPI label="Total Responses" value={String(stats.count)} />
            <KPI
              label="Testimonials (consented)"
              value={String(mine.filter((x) => x.consent).length)}
            />
          </div>
        </section>
        ...
        <section style={section}>
          {filtered.length === 0 ? (
            <div style={{ color: '#90A4AE', fontSize: 14 }}>
              <p style={{ margin: 0 }}>
                No feedback yet for this Coach ID or your filters are too narrow.
              </p>
              <p style={{ margin: '2px 0 0' }}>
                Share your public form link above to start collecting responses.
              </p>
            </div>
          ) : (
...
function KPI({ label, value }) {
  return (
    ...
    >
      ...
      <div style={{ fontSize: 22, fontWeight: 800, color: '#263238' }}>
        {value}
      </div>
    </div>
  );
