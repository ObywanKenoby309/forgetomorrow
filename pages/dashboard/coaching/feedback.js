...
export default function CoachingFeedbackDashboardPage() {
  ...
  return (
    ...
    >
      <CoachingSidebar active="feedback" />
                ...
                {filtered.map((f) => (
                  <div key={f.id} style={card}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12 }}>
                      <div>
                        <strong style={{ color: '#263238' }}>{'★'.repeat(f.rating)}{'☆'.repeat(5 - f.rating)}</strong>
                    ...
                    {f.comment && <div style={{ color: '#455A64', marginTop: 6, whiteSpace: 'pre-wrap' }}>{f.comment}</div>}
                  </div>
                ))}
              </div>
            )}
...
function KPI({ label, value }) {
  return (
    <div style={{ background: '#FFFFFF', border: '1px solid #eee', borderRadius: 10, padding: 12, minHeight: 70, display: 'grid', gap: 4 }}>
...
const section = { background: 'white', borderRadius: 12, padding: 20, boxShadow: '0 2px 6px rgba(0,0,0,0.06)', border: '1px solid #eee' };
const input   = { border: '1px solid #ddd', borderRadius: 10, padding: '10px 12px', outline: 'none', background: 'white' };
