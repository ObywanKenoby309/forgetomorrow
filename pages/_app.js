...
export default function App({ Component, pageProps }) {
  ...
  const isSeekerRoute =
    ...
    [
      '/the-hearth', '/jobs', '/applications', '/pinned-jobs',
      '/resume-cover', '/roadmap', '/profile', '/profile-analytics', '/feed'
  ...
  const sharedRoutes = new Set([
    '/help', '/privacy', '/terms',
    '/security', '/accessibility', '/cookies'
  ]);
  ...
  const isPublicEffective =
    sharedRoutes.has(router.pathname)
  ...
  const forgeBgPosition =
    router.pathname === '/' ? '35% center' : 'center';

  const renderLandingHeader =
    isPublicEffective && !isUniversalPage;
