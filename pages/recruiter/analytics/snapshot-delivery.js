...
const FALLBACK_TIMEZONES = [
  "UTC","America/Anchorage","America/Los_Angeles","America/Phoenix","America/Denver","America/Chicago","America/New_York",
  "America/Toronto","America/Halifax","America/Sao_Paulo","America/Buenos_Aires","Europe/London","Europe/Paris","Europe/Berlin",
  "Europe/Helsinki","Europe/Istanbul","Africa/Cairo","Africa/Johannesburg","Africa/Lagos","Africa/Nairobi","Asia/Jerusalem",
  "Asia/Riyadh","Asia/Dubai","Asia/Karachi","Asia/Kolkata","Asia/Bangkok","Asia/Singapore","Asia/Shanghai","Asia/Tokyo",
...
function prettyTZ(tz) {
  ...
  const label = TIMEZONE_LABEL_MAP[tz] || "Time Zone";
...
function defaultSchedule(reportType) {
  return {
    reportType,
    recipients: "",
    cadence: "weekly",
...
function FieldLabel({ children }) {
  return <div style={{ fontSize: 12, fontWeight: 800, color: SLATE, marginBottom: 6 }}>{children}</div>;
}
  ...
  return (
    ...
    >
      <div>
        ...
        {hint && <div style={{ fontSize: 12, color: MUTED, marginTop: 4, lineHeight: 1.5 }}>{hint}</div>}
      </div>
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div style={{ fontSize: 14, color: MUTED, marginBottom: 4 }}>
        Each report has its own schedule, recipients, and timing. Click any report to configure it.
      </div>
      {REPORTS.map((report) => {
          ...
          >
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                ...
                {hasSchedule ? (
                  <span style={{ fontSize: 10, fontWeight: 800, background: "rgba(255,112,67,0.12)", color: ORANGE, borderRadius: 6, padding: "2px 8px" }}>
                    Active
                  </span>
                ) : (
                  <span style={{ fontSize: 10, fontWeight: 800, background: "rgba(100,116,139,0.10)", color: MUTED, borderRadius: 6, padding: "2px 8px" }}>
                    Not configured
                  </span>
                )}
              </div>
...
function ReportScheduleEditor({
  ...
  useEffect(() => {
    setS(schedule || defaultSchedule(report.key));
  }, [report.key, schedule]);
  ...
  const filteredTZs = useMemo(() => {
    ...
    return allTimezones.filter(
      (tz) => tz.toLowerCase().includes(q) || (TIMEZONE_LABEL_MAP[tz] || "").toLowerCase().includes(q)
    );
  ...
  return (
    <div style={workspaceStyle}>
      <div style={{ ...GLASS, borderRadius: 18, padding: 16, ...(isMobile ? {} : { gridArea: "recipients" }) }}>
        ...
        <div style={{ fontSize: 13, color: MUTED, marginBottom: 12 }}>{report.description}</div>
        <Textarea
          value={s.recipients}
        ...
        <div style={{ marginTop: 10, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}>
          <ToggleRow
            checked={s.sendToSelf}
            onChange={(e) => update({ sendToSelf: e.target.checked })}
            label="Send a copy to me"
            hint="Get a confirmation copy on every send."
          />
      ...
      <div style={{ ...GLASS, borderRadius: 18, padding: 16, ...(isMobile ? {} : { gridArea: "schedule" }) }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap", marginBottom: 14 }}>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            {["daily", "weekly", "monthly"].map((c) => (
              <PillButton key={c} active={s.cadence === c} onClick={() => update({ cadence: c })} small>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
          <div>
            ...
            <Input type="text" value={tzSearch} onChange={(e) => setTzSearch(e.target.value)} placeholder="Search city or zone..." />
          </div>
          <div>
            <FieldLabel>Time zone</FieldLabel>
            <Select value={s.timezone} onChange={(e) => update({ timezone: e.target.value })} size={5}>
              {filteredTZs.map((tz) => (
        ...
        <div style={{ fontSize: 12, color: MUTED, marginBottom: 14 }}>
          ...
          {s.timezone !== LOCAL_TZ && (
            <button
              type="button"
              onClick={() => update({ timezone: LOCAL_TZ })}
              style={{
                marginLeft: 10,
                fontSize: 11,
                fontWeight: 700,
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <div>
            ...
            <Input type="time" value={s.timeOfDay} onChange={(e) => update({ timeOfDay: e.target.value })} />
          </div>
          <div>
            {s.cadence === "weekly" && (
              <>
                <FieldLabel>Day of week</FieldLabel>
                <Select value={s.weeklyDay} onChange={(e) => update({ weeklyDay: e.target.value })}>
            ...
            {s.cadence === "monthly" && (
              <>
                ...
                <div style={{ display: "flex", borderRadius: 10, border: "1px solid rgba(51,65,85,0.14)", overflow: "hidden", marginBottom: 10 }}>
                  {[{ value: "date", label: "Specific date" }, { value: "ordinal", label: "Ordinal weekday" }].map((opt) => (
                    <button
                      key={opt.value}
                      type="button"
                      onClick={() => update({ monthlyMode: opt.value })}
                      style={{
                        flex: 1,
                        padding: "8px 10px",
                        border: "none",
                        background: s.monthlyMode === opt.value ? SLATE : "transparent",
                        color: s.monthlyMode === opt.value ? "#fff" : MUTED,
                        fontWeight: 800,
                        fontSize: 12,
                        cursor: "pointer",
                      }}
                ...
                {s.monthlyMode === "date" ? (
                  <Select value={s.monthlyDate} onChange={(e) => update({ monthlyDate: e.target.value })}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                    <Select value={s.monthlyOrdinal} onChange={(e) => update({ monthlyOrdinal: e.target.value })}>
                    ...
                    <Select value={s.monthlyWeekday} onChange={(e) => update({ monthlyWeekday: e.target.value })}>
                      {WEEKDAY_OPTIONS.map((d) => (
                        <option key={d} value={d}>
                          {d}
                        </option>
...
export default function SnapshotDeliveryPage() {
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      const loaded = {};
      const doneLoading = {};
      results.forEach(({ reportType, schedule }) => {
        loaded[reportType] = schedule;
        doneLoading[reportType] = false;
      });
      setSchedules(loaded);
      setLoadingTabs(doneLoading);
    });
  }, []);

  const handleSave = useCallback(async (scheduleData) => {
    const { reportType } = scheduleData;
    setSavingTab(reportType);
    try {
      ...
      if (data.success) {
        setSchedules((prev) => ({ ...prev, [reportType]: data.schedule }));
        alert(`${REPORTS.find((r) => r.key === reportType)?.fullLabel || reportType} schedule saved.`);
      } else {
  ...
  const handlePreview = useCallback(async (scheduleData) => {
    ...
    try {
      const res = await fetch("/api/analytics/generate-snapshot", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(scheduleData),
      });
  ...
  const handleSendNow = useCallback(async (scheduleData) => {
    ...
    try {
      ...
      if (!snapshotPayload) {
        const previewRes = await fetch("/api/analytics/generate-snapshot", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(scheduleData),
        });
      ...
      const data = await res.json();
      if (data.success) {
        alert(`${REPORTS.find((r) => r.key === reportType)?.fullLabel || reportType} sent.`);
      } else {
  ...
  return (
    <RecruiterLayout
      ...
      pageTitle="Snapshot Delivery Center"
