...
export default function MessagingPage() {
  ...
  useEffect(() => {
    ...
    async function loadUser() {
      try {
        ...
        if (!session?.user?.id) {
          if (!cancelled) { setLoadingUser(false); await router.replace("/auth/signin"); }
          return;
        }
        if (!cancelled) { setCurrentUserId(session.user.id); setLoadingUser(false); }
      } catch (err) {
        console.error("Recruiter messaging session load failed:", err);
        if (!cancelled) { setSessionError("Session did not load from /api/auth/session."); setLoadingUser(false); }
      }
    }
    loadUser();
  ...
  async function sendMessage(conversationId, text) {
    const res = await fetch("/api/messages", {
      ...
      credentials: "include",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ conversationId, content: text, channel: "recruiter" }),
    });
    if (!res.ok) {
      const payload = await res.json().catch(() => ({}));
  ...
  async function bulkSend(ids, text) {
    ...
    try {
      ...
      console.error("[MessagingPage] bulkSend error:", err);
    }
  }
      ...
      <PlanProvider>
        ...
        >
          <section style={{ ...GLASS, overflow: "hidden", padding: 16 }}>
            <div style={{ ...WHITE_CARD, minHeight: 256, display: "flex", alignItems: "center", justifyContent: "center", color: MUTED, fontSize: 14, fontWeight: 600 }}>
              Loading…
            </div>
          </section>
        </RecruiterLayout>
        ...
        >
          <section style={{ ...GLASS, overflow: "hidden", padding: 16 }}>
            <div style={{ ...WHITE_CARD, padding: 16 }}>
              <div style={{ fontWeight: 800, fontSize: 16, color: "#0F172A" }}>Session failed to load</div>
              <p style={{ marginTop: 6, fontSize: 13, color: MUTED, lineHeight: 1.55 }}>
                {sessionError || "We could not resolve your session."}
              </p>
              <div style={{ marginTop: 14, display: "flex", gap: 10 }}>
                <button style={{ borderRadius: 10, background: "#0F172A", color: "white", padding: "10px 14px", fontSize: 13, fontWeight: 700, border: "none", cursor: "pointer" }} onClick={() => window.location.reload()}>Retry</button>
                <button style={{ borderRadius: 10, border: "1px solid rgba(0,0,0,0.12)", background: "white", color: "#0F172A", padding: "10px 14px", fontSize: 13, fontWeight: 700, cursor: "pointer" }} onClick={() => router.push("/auth/signin")}>Sign in</button>
              </div>
            </div>
          </section>
        </RecruiterLayout>
