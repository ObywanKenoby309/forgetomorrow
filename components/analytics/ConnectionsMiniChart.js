...
export default function ConnectionsMiniChart({ labels = [], data = [] }) {
  ...
  const rows = useMemo(() => {
    ...
    return safeLabels.map((day, index) => ({ day, connections: Number(safeData[index] || 0) }));
  }, [labels, data]);
    ...
    <div ref={wrapRef} style={{ width: "100%", height: isMobile ? 220 : "clamp(220px, 30vw, 320px)" }}>
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          ...
          <XAxis dataKey="day" interval={0} tickMargin={isMobile ? 8 : 12} tick={{ fill: "#607D8B", fontSize: isMobile ? 10 : 12 }} />
