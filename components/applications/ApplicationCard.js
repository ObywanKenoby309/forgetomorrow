...
export default function ApplicationCard({ job, stage, onMove, onDelete }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ color: '#546E7A', fontSize: 13 }}>{job.company} • {job.location}</div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        <button
          ...
          style={{ background: 'none', border: 'none', cursor: currentIndex === 0 ? 'not-allowed' : 'pointer' }}
        >
        ...
        <button
          ...
          style={{ background: 'none', border: 'none', cursor: currentIndex === STAGES.length - 1 ? 'not-allowed' : 'pointer' }}
        >
