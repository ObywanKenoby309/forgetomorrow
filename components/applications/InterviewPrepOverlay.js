      ...
      >
        <div style={{ display: 'grid', gap: 14, minWidth: 0 }}>
          <Panel
            ...
            title="Focus First"
            count={prepAreas.length}
            subtitle="These are the areas to prepare before the interview. Build one clear STAR story for each high-focus item."
          >
            {topFocus.length ? (
              <div style={{ display: 'grid', gap: 10 }}>
                {topFocus.map((item, i) => (
          ...
          <div
            style={{
              display: 'grid',
              gridTemplateColumns: confidenceAreas.length && transferable.length ? '1fr 1fr' : '1fr',
              gap: 14,
              alignItems: 'start',
            }}
          >
        ...
        >
          ...
          >
            ...
            <div style={{ marginTop: 4, fontSize: 15, fontWeight: 950, lineHeight: 1.25 }}>
              Practice the story, not just the answer.
            </div>
          ...
          >
            {topQuestions.length ? (
              <div style={{ display: 'grid', gap: 9 }}>
                {topQuestions.map((item, i) => (
                  <QuestionCard key={`q-${i}`} item={item} compact />
                ))}
              </div>
            ) : (
              <EmptyMini message="No role-specific questions were returned. Use the story prompts below." />
            )}
          </Panel>
