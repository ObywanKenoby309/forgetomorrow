 ...
 * - gridGapClass: gap between LEFT | MIDDLE | RIGHT columns
 * - navGapClass:  gap between individual nav links
 * - centerJustifyClass: alignment of the center nav (default justify-center)
 * - containerClass/leftClass/rightClass: paddings around each zone
 */
export default function EnterpriseHeader({
  ...
  // IMPORTANT: grid gap between brand | nav | actions
  gridGapClass: gridGapClassProp,

  // Control center alignment (used only when nav is in the center column)
  ...
  // Public preset (base) — can still be overridden by props above
  ...
  // Resolve grid gap class: if none provided, use bigger gap on public, default on internal
  const gridGapClass = gridGapClassProp || (publicVariant ? "gap-12 md:gap-20" : "gap-4");

  // Suppress plan badge on coaching or when explicitly disabled
  const suppressBadge =
  ...
  // NEW: for public pages without user menu, push nav to the RIGHT column
  const navInRight = publicVariant && !showUserMenu;

  return (
    <header
      className="bg-[#2a2a2a] text-gray-300 shadow-md sticky top-0 left-0 right-0 z-[9999]"
      data-variant={publicVariant ? "public" : "internal"}
    >
      <nav
        className={`${resolvedContainer} ${resolvedHeight} grid items-center ${gridGapClass}`}
        style={{
          gridTemplateColumns: "auto 1fr auto",
          ...(publicVariant ? { gap: "3rem" } : null), // inline fallback so spacing is obvious in dev
        }}
      >
        {/* LEFT — brand */}
        ...
        {/* MIDDLE — nav (center column) */}
        {navInRight ? (
          // Empty spacer to mirror coaching layout: keeps the 1fr middle column
        ...
        {/* RIGHT — actions/profile OR (on public) NAV */}
              ...
              {showUserMenu && (
                <div className="relative" ref={profileRef}>
                  ...
                  {openProfile && (
                    <div
                      role="menu"
                      className="absolute right-0 mt-2 w-44 rounded-md border border-[#3a2a2a] bg-[#2f2f2f] shadow-lg overflow-hidden"
                    >
                      ...
                      {process.env.NODE_ENV !== "production" && (
                        <button
                          onClick={togglePlan}
                          className="w-full text-left px-3 py-2 text-xs text-gray-300 hover:bg-[#333]"
        ...
        {/* Mobile hamburger (grid item after the three columns; visible only on md-) */}
