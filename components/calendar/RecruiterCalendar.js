...
function typeColors(type) {
  ...
  if (t === 'screen' || t === 'phone screen') return { strip: '#546E7A', pillBg: '#ECEFF1', pillFg: '#263238' };
  if (t === 'offer') return { strip: '#FF7043', pillBg: '#FFF3E0', pillFg: '#C75B33' };
  if (t === 'sourcing') return { strip: '#90A4AE', pillBg: '#F5F7FB', pillFg: '#455A64' };
  if (t === 'task' || t === 'appointment') return { strip: '#B0BEC5', pillBg: '#F5F7F9', pillFg: '#37474F' };
...
function normalizeEvent(raw) {
  ...
  return {
    ...
    scope,
    calendarScope: scope,
    internalInvitees: Array.isArray(raw.internalInvitees) ? raw.internalInvitees : [],
    externalInvitees: Array.isArray(raw.externalInvitees) ? raw.externalInvitees : [],
    invitees: Array.isArray(raw.invitees) ? raw.invitees : [],
    enableVideo: !!raw.enableVideo,
    foundryRoomId: raw.foundryRoomId || null,
    foundryGuestToken: raw.foundryGuestToken || null,
    foundryJoinUrl: raw.foundryJoinUrl || null,
    foundryGuestJoinUrl: raw.foundryGuestJoinUrl || null,
    foundryScheduledAt: raw.foundryScheduledAt || null,
    foundryTimezone: raw.foundryTimezone || null,
  };
}
    ...
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', marginBottom: 4 }}>
        {WEEKDAYS_SHORT.map((d, i) => (
          <div key={i} style={{ textAlign: 'center', fontSize: 11, fontWeight: 700, color: '#90A4AE', textTransform: 'uppercase' }}>
            {d}
          </div>
        ))}
      </div>
...
function AgendaEventRow({ event, onEdit }) {
  ...
  const titleText =
    event.title ||
    (event.candidateName && event.type
      ? `${event.candidateName} – ${event.type}`
      : event.candidateName || event.type || 'Item');
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginBottom: 4 }}>
          ...
          <span style={{ fontSize: 12, color: '#607D8B', flexShrink: 0, fontWeight: 600 }}>{event.time || ''}</span>
        </div>

        {(event.company || event.jobTitle || event.req) && (
          <div style={{ fontSize: 12, color: '#78909C', marginBottom: 6, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {event.company || event.jobTitle || event.req}
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          ...
          {event.enableVideo && (
            <span style={{ fontSize: 11, background: 'rgba(26,75,143,0.10)', color: '#1A4B8F', padding: '2px 8px', borderRadius: 999, fontWeight: 700 }}>
              Audio/Video
            </span>
          )}
          {event.status && event.status !== 'Scheduled' && (
            <span style={{ fontSize: 11, background: '#F5F7F9', color: '#90A4AE', padding: '2px 8px', borderRadius: 999, fontWeight: 700 }}>
              {event.status}
            </span>
          )}
        </div>
...
// ─── Mobile: Agenda view ──────────────────────────────────────────────────────
function AgendaView({ scopedEvents, selectedDate, onEdit }) {
        ...
        return (
          <div key={dateStr}>
            ...
            >
              ...
              <div>
                <div style={{ fontWeight: 800, fontSize: 14, color: isToday ? '#FF7043' : '#112033' }}>{fmtDayLabel(dateStr)}</div>
                {evs.length > 0 && (
            ...
            {evs.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {evs.map((e) => (
                  <AgendaEventRow key={e.id || `${dateStr}-${e.time}`} event={e} onEdit={onEdit} />
                ))}
              </div>
            )}
          </div>
    ...
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 4, marginBottom: 16 }}>
        {weekDays.map(({ key, date }) => {
          ...
          return (
            ...
            >
              <span style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', color: isSelected ? 'rgba(255,255,255,0.8)' : '#90A4AE' }}>
                {WEEKDAYS_SHORT[date.getDay()]}
              </span>
              <span style={{ fontSize: 15, fontWeight: 800, color: isSelected ? 'white' : isToday ? '#FF7043' : '#112033' }}>
                {date.getDate()}
              </span>
              {hasEvents && <span style={{ width: 4, height: 4, borderRadius: 999, background: isSelected ? 'rgba(255,255,255,0.8)' : '#FF7043' }} />}
            </button>
          );
        })}
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {(eventsByDate[selectedDate] || []).length === 0 ? (
          ...
          (eventsByDate[selectedDate] || []).map((e) => <AgendaEventRow key={e.id || `${selectedDate}-${e.time}`} event={e} onEdit={onEdit} />)
        )}
      </div>
    </div>
  );
  ...
  return (
    <div style={{ position: 'relative' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12, gap: 8 }}>
        <div style={{ display: 'flex', gap: 4, background: 'rgba(255,255,255,0.70)', borderRadius: 999, padding: 3, border: '1px solid rgba(0,0,0,0.08)' }}>
          {views.map((v) => (
            <button
              key={v.key}
              type="button"
          ...
          {['personal', 'team'].map((s) => (
            <button
              ...
              style={{
                padding: '5px 10px',
                borderRadius: 999,
                border: viewScope === s ? `1px solid ${s === 'personal' ? 'rgba(255,112,67,0.30)' : 'rgba(26,75,143,0.25)'}` : '1px solid transparent',
                cursor: 'pointer',
                fontWeight: 700,
                fontSize: 11,
                background: viewScope === s ? (s === 'personal' ? 'rgba(255,112,67,0.15)' : 'rgba(26,75,143,0.12)') : 'rgba(255,255,255,0.70)',
      ...
      <MiniMonth cursor={cursor} setCursor={setCursor} selectedDate={selectedDate} onSelectDate={setSelectedDate} eventsByDate={eventsByDate} />

      {mobileView === 'agenda' && <AgendaView scopedEvents={scopedEvents} selectedDate={selectedDate} onEdit={onEdit} />}
      {mobileView === 'week' && <WeekView scopedEvents={scopedEvents} selectedDate={selectedDate} onSelectDate={setSelectedDate} onEdit={onEdit} />}
      {mobileView === 'month' && (
        <div>
          <div style={{ fontWeight: 800, fontSize: 15, color: '#112033', marginBottom: 10 }}>{fmtDayLabel(selectedDate)}</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {(eventsByDate[selectedDate] || []).length === 0 ? (
              <div
                style={{
...
export default function RecruiterCalendar({ title = 'Recruiter Calendar', seed = [], storageKey, onDaySelect, selectedDate: externalSelectedDate }) {
  ...
  const handleSave = async (formData) => {
    ...
    try {
      ...
      const payload = {
        ...
        foundryRoomId: formData.foundryRoomId || null,
        foundryGuestToken: formData.foundryGuestToken || null,
        foundryJoinUrl: formData.foundryJoinUrl || null,
        foundryGuestJoinUrl: formData.foundryGuestJoinUrl || null,
        foundryScheduledAt: formData.foundryScheduledAt || null,
        foundryTimezone: formData.foundryTimezone || null,
      };
      ...
      try {
        ...
        console.warn('Save API error:', err);
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      try {
        await fetch('/api/recruiter/calendar', {
          method: 'DELETE',
          headers: { 'Content-Type': 'application/json' },
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 2, minWidth: 0 }}>
                          ...
                          <div style={{ display: 'flex', gap: 4, marginTop: 2, flexWrap: 'wrap' }}>
                            ...
                            <span
                              style={{
                                fontSize: 10,
                                background: e.scope === 'personal' ? 'rgba(255,112,67,0.12)' : '#ECEFF1',
                                color: e.scope === 'personal' ? '#C75B33' : '#607D8B',
                                padding: '2px 6px',
                                borderRadius: 999,
                                lineHeight: 1.2,
                            ...
                            {e.enableVideo && (
                              <span style={{ fontSize: 10, background: 'rgba(26,75,143,0.10)', color: '#1A4B8F', padding: '2px 6px', borderRadius: 999, lineHeight: 1.2 }}>
                                AV
                              </span>
                            )}
                          </div>
