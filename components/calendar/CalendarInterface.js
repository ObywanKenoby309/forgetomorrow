 ...
 * - seed: array of { date:'YYYY-MM-DD', time:'HH:mm', client:string, type:string, status:string }
 * - typeChoices: string[] — e.g., ['Strategy','Resume','Interview'] or ['Interview','Application','Task']
 * - statusChoices: string[] — e.g., ['Scheduled','Completed','No-show']
 * - title?: string — optional heading shown above toolbar
 * - backHref?: string — optional link shown in the toolbar (e.g., “← Back to Agenda”)
 * - addLabel?: string — button label for add (default: “+ Add Item”)
 */
    ...
    <div style={{ display: 'grid', gap: 16 }}>
      ...
      >
        <div style={{ display: 'grid', gridTemplateColumns: backHref ? '1fr 1fr auto auto' : '1fr 1fr auto', gap: 12, alignItems: 'center' }}>
          ...
          {backHref && (
            <a href={backHref} style={{ color: '#FF7043', fontWeight: 700 }}>← Back</a>
          )}
        ...
        >
          ...
          >
            <div style={{ fontWeight: 800, color: '#263238', marginBottom: 4 }}>
              {modal.mode === 'add' ? 'Add Calendar Item' : 'Edit Calendar Item'}
            </div>
            ...
            <div>
              ...
              <input value={form.client} onChange={(e) => update('client', e.target.value)} style={input} placeholder="e.g., Phone Screen — Acme" required />
            </div>
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'space-between', marginTop: 6 }}>
              {modal.mode === 'edit' ? (
                <button
                  type="button"
                  onClick={() => deleteItem(modal.index)}
                  style={{
                    background: 'white',
                    color: '#C62828',
