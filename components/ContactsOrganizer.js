...
export default function ContactsOrganizer({
  ...
  const getCategoryTotalCount = useMemo(() => {
    return (categoryId) => {
      if (!categoryId) return 0;

      const idsToVisit = [String(categoryId)];
      const categoryIdsInTree = new Set();

      while (idsToVisit.length) {
        const currentId = idsToVisit.pop();
        if (!currentId || categoryIdsInTree.has(currentId)) continue;

        categoryIdsInTree.add(currentId);

        sortedCategories.forEach((cat) => {
          if (String(cat.parentCategoryId || '') === currentId) {
            idsToVisit.push(String(cat.id));
          }
        });
      }
      ...
      localAssignments.forEach((assignment) => {
        const assignmentCategoryId = String(assignment?.categoryId || '');
        const assignmentContactId = String(assignment?.contactId || '');
        if (!assignmentCategoryId || !assignmentContactId) return;

        if (categoryIdsInTree.has(assignmentCategoryId)) {
          uniqueContactIds.add(assignmentContactId);
        }
      });

      return uniqueContactIds.size;
    };
  }, [sortedCategories, localAssignments]);
    ...
    <div style={{ display: 'grid', gap: 20, width: '100%' }}>
      ...
      <section style={{ display: 'grid', gap: 20 }}>
        <CategoryBlock
          ...
          totalCount={unassignedContacts.length}
          ...
          return (
            <div key={root.id} style={{ display: 'grid', gap: 12 }}>
              <CategoryBlock
                ...
                totalCount={getCategoryTotalCount(root.id)}
                allCategories={sortedCategories}
                rootCategories={rootCategories}
                onAssign={assignContact}
                onUnassign={unassignContact}
                ...
                root.children.map((child) => {
                  ...
                  return (
                    <CategoryBlock
                      ...
                      totalCount={getCategoryTotalCount(child.id)}
                      allCategories={sortedCategories}
                      rootCategories={rootCategories}
                      onAssign={assignContact}
                      onUnassign={unassignContact}
