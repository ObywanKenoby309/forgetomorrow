// components/recruiter/RecruiterInbox.js
          ...
          <MessageThread
            ...
            inboxDescription={
              <p className="mt-1 text-[11px] text-slate-500 leading-snug">
                Conversations you start as a <span className="font-semibold">Recruiter</span>{" "}
                will show here. Personal DMs live in{" "}
                <span className="font-semibold">The Spark</span>.
              </p>
            }
            emptyTitle="No recruiter conversations yet"
            emptyBody={
              <>
                <p className="max-w-md text-xs text-slate-500">
                  This inbox is for conversations you start as{" "}
                  <span className="font-semibold">Recruiter</span>. To begin, open a candidate
...
function RecruiterInboxContainer() {
  ...
  const onBulkSendDb = async (ids, text) => {
    ...
    try {
      ...
      console.error("Failed bulk send (recruiter):", err);
      setBulkOpen(false);
    }
  };
  ...
  return (
    <RecruiterInbox
      ...
      onActiveThreadChange={(t) => {
        setActiveThread(t);
        setIsBlocked(false);
        setActiveHomeLocation(t?.homeLocation || "recruiter");
      }}
      isBlocked={isBlocked}
...
export default RecruiterInboxContainer;
