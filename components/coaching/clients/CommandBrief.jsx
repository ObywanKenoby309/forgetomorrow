...
// Tabbed briefing system. Not a scroll container — a navigation model.
//
// Tabs: Overview · Direction · Risks · Targets · Execution
// Reasoning: collapsible, present on every tab

import React, { useState } from 'react';

const TABS = ['overview', 'direction', 'risks', 'targets', 'execution'];
...
export default function CommandBrief({ clientName, generatedAt, strategyBrief, onEditInputs, onFeedback }) {
  ...
  return (
    <div className="w-full space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pb-3 border-b border-white/30">
        <div>
          <div className="text-[11px] font-bold tracking-[0.10em] text-slate-400 uppercase mb-0.5">Coaching Brief</div>
          <div className="text-[17px] font-black tracking-tight text-slate-900">{clientName || 'Client'}</div>
          {generatedAt ? (
  ...
  {t.reason && (
    <div className="text-xs text-slate-600">
      ...
      {/* Reasoning — collapsible, global */}
      {b.reasoning?.length > 0 && (
        <div className="mt-6 rounded-2xl border border-slate-200 bg-white/60">
          <button type="button" onClick={() => setReasoningOpen((v) => !v)} className="w-full flex items-center justify-between px-4 py-3 text-left">
            <span className="text-[10px] font-black tracking-[0.10em] text-slate-400 uppercase">Why This Strategy</span>
      ...
      <div className="flex items-center justify-between pt-2 border-t border-white/30">
        <div className="flex items-center gap-2">
          {feedbackSent ? (
            <span className="text-[11px] text-slate-400">{feedbackSent === 'up' ? 'Thanks for the feedback 👍' : 'Feedback noted 👎'}</span>
          ) : (
            <>
              <span className="text-[11px] text-slate-400 mr-1">Was this useful?</span>
              <button type="button" onClick={handleUp} className="px-2.5 py-1.5 rounded-lg border border-slate-200 bg-white/80 text-slate-700 text-xs hover:bg-white transition">👍 Useful</button>
              <button type="button" onClick={handleDown} className="px-2.5 py-1.5 rounded-lg border border-slate-200 bg-white/80 text-slate-700 text-xs hover:bg-white transition">👎 Needs Work</button>
            </>
          )}
        </div>
        <button type="button" onClick={onEditInputs} className="text-[12px] font-semibold text-slate-400 hover:text-[#FF7043] transition">← Edit inputs & regenerate</button>
      </div>
