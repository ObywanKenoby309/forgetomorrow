// components/resume-form/templates/HybridDesignedPDF.js
...
function fmtMonth(val) {
  const s = safe(val);
  if (!s) return '';
  // supports YYYY-MM or YYYY-MM-DD or YYYY
...
function normalizeBullets(exp) {
  ...
  // If description is multi-line, treat each line as a bullet (common in your builder)
...
export default function HybridDesignedPDF({ data }) {
