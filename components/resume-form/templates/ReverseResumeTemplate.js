              ...
              <div key={i} style={{ marginBottom:'8pt',...(isEditMode?editBox:{}) }}>
                {isEditMode ? (
                  ...
                  <>
                    ...
                    {((cert.issuer||cert.organization)||getCertificationDate(cert))&&<div style={{ fontSize:'10pt',color:'#444' }}>{[cert.issuer||cert.organization,getCertificationDate(cert)].filter(Boolean).join(' • ')}</div>}
                  </>
                )}
              </div>
            ))}
