  ...
  if (multiline) {
    return (
      <textarea value={value||''} onChange={(e)=>onChange(e.target.value)} placeholder={placeholder} rows={3}
        style={{ width:'100%',border:`1.5px solid ${ORANGE}`,borderRadius:6,padding:'6px 8px',fontSize:'inherit',fontFamily:'inherit',lineHeight:'inherit',color:'inherit',background:'rgba(255,112,67,0.04)',resize:'vertical',outline:'none',boxSizing:'border-box',...style }} />
    );
  }
  return (
    <input type="text" value={value||''} onChange={(e)=>onChange(e.target.value)} placeholder={placeholder}
      style={{ border:`1.5px solid ${ORANGE}`,borderRadius:6,padding:'4px 8px',fontSize:'inherit',fontFamily:'inherit',color:'inherit',background:'rgba(255,112,67,0.04)',outline:'none',width:'100%',boxSizing:'border-box',...style }} />
  );
}
...
function SectionWrapper({ isEditMode, label, children, onAdd, addLabel }) {
  return (
    <div style={{ marginBottom:'16pt',position:'relative' }}>
      {isEditMode&&<div style={{ position:'absolute',top:0,right:0,fontSize:10,color:ORANGE,fontWeight:700,background:'rgba(255,112,67,0.08)',border:`1px solid rgba(255,112,67,0.25)`,borderRadius:4,padding:'2px 7px',userSelect:'none' }}>✏️ {label}</div>}
      {children}
      {isEditMode&&onAdd&&<button type="button" onClick={onAdd} style={{ marginTop:8,background:'rgba(255,112,67,0.10)',border:`1px solid rgba(255,112,67,0.30)`,color:ORANGE,borderRadius:6,padding:'4px 12px',fontSize:11,fontWeight:800,cursor:'pointer' }}>+ {addLabel||'Add'}</button>}
    </div>
  );
}

const H2 = { fontSize:'13pt',fontWeight:'bold',margin:'0 0 6pt 0',borderBottom:'1pt solid #000',textTransform:'uppercase' };
const editBox = { border:'1px solid rgba(255,112,67,0.15)',borderRadius:8,padding:'10px',background:'rgba(255,112,67,0.02)' };

export default function HybridResumeTemplate({ data, isEditMode = false, onUpdate }) {
  const {
  ...
  return (
    <div style={{ width:'100%',padding:0,margin:0,fontFamily:'Helvetica, Arial, sans-serif',fontSize:'11pt',lineHeight:'1.4',color:'#1f2937' }}>
      ...
      <SectionWrapper isEditMode={isEditMode} label="Contact Info">
        <div style={{ textAlign:isEditMode?'left':'center',marginBottom:'20pt',paddingTop:isEditMode?28:0 }}>
          {isEditMode ? (
            <div style={{ display:'grid',gap:6 }}>
              ...
              <EditableText isEditMode value={personalInfo.targetedRole} onChange={(v)=>updatePI('targetedRole',v)} placeholder="Targeted Role" />
            ...
            <>
              ...
              {personalInfo.targetedRole&&<p style={{ fontSize:'12pt',fontStyle:'italic',margin:'8pt 0 0 0',color:'#444' }}>{personalInfo.targetedRole}</p>}
            </>
          )}
        </div>
      </SectionWrapper>
      ...
      <SectionWrapper isEditMode={isEditMode} label="Summary">
        {(summary||isEditMode)&&(
          <div style={{ marginBottom:'16pt' }}>
            ...
            <EditableText isEditMode={isEditMode} value={summary} onChange={(v)=>update('summary',v)} placeholder="Write a compelling professional summary..." multiline style={{ margin:0,fontSize:'11pt' }} />
          </div>
        )}
      </SectionWrapper>

      {/* SKILLS — full width, before experience (Hybrid style) */}
          ...
          <div style={{ marginBottom:'16pt' }}>
            ...
            {isEditMode ? (
              ...
              <div style={{ columns:2,columnGap:'12pt' }}>
                {skills.map((skill,i)=><p key={i} style={{ margin:'2pt 0',fontSize:'11pt',breakInside:'avoid' }}>• {skill}</p>)}
              </div>
            )}
          </div>
        )}
              ...
              <div key={i} style={{ marginBottom:'12pt',...(isEditMode?editBox:{}) }}>
                {isEditMode ? (
                  <div style={{ display:'grid',gap:6 }}>
                    <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:6 }}>
                      ...
                      <div style={{ display:'flex',gap:4 }}>
                        ...
                        <EditableText isEditMode value={exp.endDate} onChange={(v)=>{ const u=[...workExperiences]; u[i]={...exp,endDate:v}; update('workExperiences',u); }} placeholder="Present" />
                      </div>
                    </div>
                    <div>
                      {(exp.bullets||['']).map((b,bi)=>(
                        <div key={bi} style={{ display:'flex',gap:6,marginBottom:4 }}>
                          ...
                          <EditableText isEditMode value={b} onChange={(v)=>{ const u=[...workExperiences]; const bl=[...(exp.bullets||[])]; bl[bi]=v; u[i]={...exp,bullets:bl}; update('workExperiences',u); }} placeholder="Achievement..." multiline style={{ fontSize:'10pt' }} />
                          <button type="button" onClick={()=>{ const u=[...workExperiences]; u[i]={...exp,bullets:(exp.bullets||[]).filter((_,x)=>x!==bi)}; update('workExperiences',u); }} style={{ color:'#EF4444',background:'none',border:'none',cursor:'pointer',flexShrink:0,fontSize:16 }}>×</button>
                        </div>
                      ))}
                  ...
                  <>
                    ...
                    {(exp.bullets||[]).map((b,bi)=><p key={bi} style={{ margin:'2pt 0 2pt 16pt',fontSize:'11pt' }}>• {b}</p>)}
                  </>
                )}
              </div>
            ))}
                ...
                {isEditMode ? (
                  <div style={{ display:'grid',gap:6 }}>
                    <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:6 }}>
                      ...
                      <EditableText isEditMode value={proj.startDate} onChange={(v)=>{ const u=[...projects]; u[i]={...proj,startDate:v}; update('projects',u); }} placeholder="Start" />
                      <EditableText isEditMode value={proj.endDate} onChange={(v)=>{ const u=[...projects]; u[i]={...proj,endDate:v}; update('projects',u); }} placeholder="End" />
                    </div>
                    {(proj.bullets||['']).map((b,bi)=>(
                      <div key={bi} style={{ display:'flex',gap:6,marginBottom:4 }}>
                        <span style={{ color:'#94A3B8',paddingTop:6,flexShrink:0 }}>•</span>
                        <EditableText isEditMode value={b} onChange={(v)=>{ const u=[...projects]; const bl=[...(proj.bullets||[])]; bl[bi]=v; u[i]={...proj,bullets:bl}; update('projects',u); }} placeholder="Detail..." multiline style={{ fontSize:'10pt' }} />
                  ...
                  <>
                    <div style={{ display:'flex',justifyContent:'space-between',alignItems:'flex-start' }}>
                      <div>
                        <strong style={{ fontSize:'11pt' }}>{proj.title||proj.name}</strong>
                        <span style={{ color:'#444',marginLeft:'8pt' }}>{proj.company||proj.org||proj.client}{proj.location&&` • ${proj.location}`}</span>
                      </div>
                      {(proj.startDate||proj.endDate)&&<span style={{ fontSize:'10pt',color:'#666',whiteSpace:'nowrap' }}>{proj.startDate} – {proj.endDate||'Present'}</span>}
                    </div>
                    {(proj.bullets||[]).map((b,bi)=><p key={bi} style={{ margin:'2pt 0 2pt 16pt',fontSize:'11pt' }}>• {b}</p>)}
                    {proj.description&&(!proj.bullets||proj.bullets.length===0)&&<p style={{ margin:'2pt 0 2pt 16pt',fontSize:'11pt' }}>{proj.description}</p>}
                  </>
                )}
              </div>
            ))}
              ...
              <div key={i} style={{ marginBottom:'10pt',...(isEditMode?editBox:{}) }}>
                {isEditMode ? (
                  <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:6 }}>
                    ...
                    <EditableText isEditMode value={edu.school||edu.institution} onChange={(v)=>{ const u=[...educationList]; u[i]={...edu,school:v,institution:v}; update('educationList',u); }} placeholder="School" />
                  ...
                  <>
                    <div style={{ fontWeight:'bold',fontSize:'11pt' }}>{edu.degree} {edu.field&&`${edu.field}`}</div>
                    <div style={{ fontSize:'10pt' }}>{(edu.institution||edu.school)}{edu.location&&` • ${edu.location}`}</div>
                    <div style={{ fontSize:'10pt',color:'#666',whiteSpace:'nowrap' }}>{edu.startDate} – {edu.endDate||'Present'}</div>
        ...
        return (
          <div key={i} style={{ marginBottom:'16pt' }}>
            ...
            {items&&items.length>0 ? (
              <ul style={{ margin:0,paddingLeft:'16pt',fontSize:'11pt' }}>
                {items.map((item,idx)=><li key={idx} style={{ marginBottom:'3pt' }}>{item}</li>)}
              </ul>
            ) : (content&&<p style={{ margin:0,fontSize:'11pt' }}>{content}</p>)}
          </div>
        );
      })}
    </div>
