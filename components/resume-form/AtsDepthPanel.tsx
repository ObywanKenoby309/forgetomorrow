      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
          ...
          <button
            type="button"
            onClick={() => setExpanded((v) => !v)}
            style={{ border: 'none', background: 'transparent', fontSize: 13, color: '#607D8B', cursor: 'pointer' }}
          >
            {expanded ? 'Collapse' : 'Expand'}
          </button>
        </div>

        <div style={{ marginTop: 14, background: '#E3F2FD', border: '1px solid #BBDEFB', borderRadius: 12, padding: 12 }}>
          <div style={{ fontWeight: 800, color: '#0D47A1', fontSize: 13 }}>Loaded job</div>
        ...
        <div style={{ marginTop: 16, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          ...
          >
            <div>
              ...
              <div style={{ marginTop: 14, display: 'flex', alignItems: 'center', gap: 10, fontSize: 12, color: '#607D8B' }}>
                <span style={{ width: 44, height: 4, borderRadius: 999, background: '#CFD8DC', display: 'inline-block' }} />
                <span>Uses AI + keyword coverage.</span>
              </div>

              {aiError && <div style={{ marginTop: 10, fontSize: 12, color: '#C62828' }}>{aiError}</div>}
            </div>
        ...
        {!expanded ? null : (
          <>
            {(aiUpgrade || (aiScore !== null && !aiLoading) || normalizedTips.length > 0) && (
              <div style={{ marginTop: 16, padding: 14, borderRadius: 12, border: '1px solid #FFE0B2', background: '#FFF3E0' }}>
                {aiUpgrade ? (
                  <>
                    <div style={{ fontWeight: 800, color: '#E65100', marginBottom: 6 }}>
                      You&apos;ve used your free AI scans for this month.
            ...
            <div style={{ marginTop: 20, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: 8 }}>
              {buckets.map((b) => (
                <div key={b.key} style={{ padding: 10, borderRadius: 10, border: '1px solid #ECEFF1', background: '#FAFAFA', fontSize: 12 }}>
                  <div style={{ fontWeight: 700, color: '#37474F', marginBottom: 2 }}>{b.label}</div>
                  <div style={{ color: '#607D8B' }}>{b.total > 0 ? `${b.matched}/${b.total} matched` : '0/0 matched'}</div>
                  <div style={{ marginTop: 2, fontWeight: 600, color: '#455A64' }}>{b.points} pts</div>
                </div>
              ))}
            </div>
            ...
            <div style={{ marginTop: 18, borderTop: '1px solid #ECEFF1', paddingTop: 14 }}>
              <details>
                <summary style={{ cursor: 'pointer', fontSize: 14, fontWeight: 600, color: '#37474F', listStyle: 'none' }}>
                  View keyword breakdown
                </summary>

                <div style={{ marginTop: 12, display: 'grid', gap: 10 }}>
                  <div style={{ padding: 12, borderRadius: 10, border: '1px solid #ECEFF1', background: '#FFFFFF' }}>
                    <div style={{ fontWeight: 700, marginBottom: 4, color: '#263238' }}>
                      High-impact title / role terms
                    </div>

                    {titleKeywords.length === 0 ? (
                      <p style={{ margin: 0, fontSize: 13, color: '#388E3C' }}>No missing keywords here — nice!</p>
                    ) : missingTitleKeywords.length === 0 ? (
                      <p style={{ margin: 0, fontSize: 13, color: '#388E3C' }}>No missing title/role keywords — strong alignment.</p>
                    ) : (
                      <>
                        <p style={{ margin: 0, fontSize: 13, color: '#546E7A' }}>
                          Consider weaving in these terms somewhere in your summary, skills, or experience section:
                  ...
                  {['High-impact hard skills', 'Tools / Platforms', 'Education', 'Soft skills'].map((label) => (
                    <div key={label} style={{ padding: 12, borderRadius: 10, border: '1px solid #ECEFF1', background: '#FFFFFF' }}>
                      <div style={{ fontWeight: 700, marginBottom: 4, color: '#263238' }}>{label}</div>
                      <p style={{ margin: 0, fontSize: 13, color: '#388E3C' }}>No missing keywords here — nice!</p>
                    </div>
                  ))}
                </div>
