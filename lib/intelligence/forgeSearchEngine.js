...
import {
  ...
  CAPABILITY_CLUSTERS,
  JOB_INTENT_FAMILIES,
  CONFLICTING_ECOSYSTEMS,
  getJobSearchIntent,
} from '@/lib/intelligence/searchTaxonomy';
...
const customerSupportHits =
  ...
  const searchIntent = getJobSearchIntent(intent.jobTitle || intent.q);
  const intentClusterKeys = new Set([
    ...(searchIntent.primaryClusters || []),
    ...(searchIntent.adjacentClusters || []),
  ]);

  const isDesktopSupportIntent = intentClusterKeys.has("desktopSupport");
  const isCybersecurityIntent = intentClusterKeys.has("cybersecurity");
  const isCloudIntent = intentClusterKeys.has("cloudInfrastructure");
  const isSoftwareIntent = intentClusterKeys.has("softwareEngineering");
  const isCustomerSuccessIntent = intentClusterKeys.has("customerSuccess");
  const isCustomerSupportIntent = intentClusterKeys.has("customerSupport");
