...
function scoreCandidate(candidate, filters = {}) {
  ...
  if (intent.jobTitle || intent.q) {
    ...
    if (roleHits > 0) {
	  ...
	  const points = exactTitleHit ? 45 : Math.min(35, 15 + roleHits * 8);
      score += points;
  ...
  const isSupportIntent = intent.roleTerms.some((term) =>
    [
      "desktop support",
      "desktop technician",
      "desktop tech",
  ...
  if (isSupportIntent) {
    const toolHits = supportToolTerms.filter((term) =>
      includesAny(searchableText, [term])
    );

    if (toolHits.length) {
      const points = Math.min(25, toolHits.length * 5);
      score += points;
      reasons.push("Endpoint support tool ecosystem aligns with recruiter intent.");
      evidence.push({
        type: "toolEcosystem",
        label: "Endpoint support tools",
        text: toolHits.join(", "),
        points,
      });
    }
