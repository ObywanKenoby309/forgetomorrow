...
function normalizeJob(job) {
  ...
  return {
    ...
    publishedat:
      j.publishedat || j.publishedAt || j.createdAt || null,
  };
}

function inferJobLocationType(location) {
...
function scoreJobSearch(job, filters = {}) {
  const j = normalizeJob(job);

  const keyword = safe(filters.keyword || filters.q);
  const company = safe(filters.company);
  const location = safe(filters.location);
  const locationType = safe(filters.locationType);
  const source = safe(filters.source);

  const keywordTerms = expandTerms([keyword]);
  ...
  // Role/title + semantic keyword scoring
  if (keywordTerms.length) {
    const titleHits = countMatches(j.title, keywordTerms);

    const descriptionHits =
      countMatches(j.description, keywordTerms) +
      countMatches(j.tags, keywordTerms);
    ...
    if (descriptionHits > 0) {
      ...
      evidence.push({
        ...
        text: `${descriptionHits} related signals found`,
        points,
      });
    }
  }
  ...
  // Company
  if (company && includesAny(j.company, [company])) {
    score += 10;
  ...
  // Location
  if (location && includesAny(j.location, [location])) {
    score += 10;
  ...
  if (
    locationType &&
    inferJobLocationType(j.location) === locationType
  ) {
    score += 8;
   ...
   // Ecosystem scoring must follow search intent.
  const intentClusterKeys = [];

  const keywordLower = lower(keyword);

  if (
    keywordLower.includes('customer support') ||
    keywordLower.includes('customer service') ||
    keywordLower.includes('support manager') ||
    keywordLower.includes('technical support') ||
    keywordLower.includes('help desk') ||
    keywordLower.includes('service desk')
  ) {
    intentClusterKeys.push('customerSupport', 'desktopSupport');
  }

  if (
    keywordLower.includes('customer success') ||
    keywordLower.includes('client success') ||
    keywordLower.includes('account manager')
  ) {
    intentClusterKeys.push('customerSuccess');
  }

  if (
    keywordLower.includes('frontend') ||
    keywordLower.includes('developer') ||
    keywordLower.includes('software') ||
    keywordLower.includes('engineer')
  ) {
    intentClusterKeys.push('softwareEngineering');
  }
  ...
  if (
    ...
    intentClusterKeys.push('cloudInfrastructure');
  }

  const clusterEntries =
    intentClusterKeys.length > 0
      ? Object.entries(CAPABILITY_CLUSTERS).filter(([key]) =>
          intentClusterKeys.includes(key)
        )
      : Object.entries(CAPABILITY_CLUSTERS);

  const disallowedClusterKeywords = {
    customerSuccess: [
      'react',
      'next.js',
      'typescript',
  ...
  const clusterResults = clusterEntries
    .map(([key, terms]) => {
      const blockedTerms =
  disallowedClusterKeywords[key] || [];

const blocked =
  blockedTerms.filter((term) =>
    includesAny(searchableText, [term])
  ).length >= 3;
...
const hits = terms.filter((term) =>
  ...
  // Source filter
  if (source) {
    const jobSource = lower(j.source);

    if (jobSource.includes(lower(source))) {
      score += 6;
    }
  }
...
export function rankJobsBySearchRelevance(
  jobs = [],
  filters = {}
) {
  return (Array.isArray(jobs) ? jobs : [])
    .map((job) => scoreJobSearch(job, filters))
    .sort(
      (a, b) =>
