...
function extractRelevantWins(profile, history) {
  try {
    ...
    if (latestRoadmap?.data && typeof latestRoadmap.data === 'object') {
      ...
      const combined = [...actions, ...metrics].slice(0, 6);
      combined.forEach(a => {
        ...
        const isPivot = /pivot|career change|transition|switch|explore.*role|new (field|industry|path)/i.test(label);
        if (!isPivot) wins.push(truncate(label, 120));
      });
    }
