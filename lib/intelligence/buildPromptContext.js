...
//
// ForgeTomorrow Unified Career Intelligence — Stage 4 / 5A
...
//
// Modes:
//   (none)       → full promptContext — all fields
//   alignment    → resume vs JD match focus: proofSignals, knownGaps, cautionFlags
//   execution    → action-oriented: relevantWins, recentToolInsights, positioning
//   strategy     → career direction: positioning, knownGaps, recentToolInsights
//   negotiation  → compensation/offer: user, positioning, proofSignals, recentToolInsights
//   recruiter    → recruiter lens: user, proofSignals, relevantWins, sourceStatus
//   coach        → coaching focus: knownGaps, cautionFlags, recentToolInsights, positioning
//   matching     → signal matching: proofSignals, positioning, sourceStatus
//   growth       → growth & pivot: positioning, proofSignals, relevantWins, knownGaps, cautionFlags, recentToolInsights
//
// Usage:
//   const prompt = await buildPromptContext({ userId });
//   const prompt = await buildPromptContext({ userId, mode: 'alignment' });
...
// ─── Credentials ───────────────────────────────────────────────────────────────
// Structured education + certification extraction from profile and resume.
// Canonical source for tools that need credential-specific data.
function extractCredentials(profile, resume) {
  try {
    const education = new Set();
    const certifications = new Set();

    sa(profile.education).forEach((edu) => {
      if (!edu) return;
      ...
      const label = typeof edu === 'string'
        ...
        : [
            ...
            edu.startYear && edu.endYear ? `${edu.startYear}-${edu.endYear}` : edu.year || edu.endYear || '',
          ].filter(Boolean).join(' - ');

      if (ss(label)) education.add(ss(label));
    });

    sa(profile.certifications).forEach((cert) => {
      if (!cert) return;

      const label = typeof cert === 'string'
        ? cert
        : [
            cert.name || cert.certification || cert.title,
            cert.issuer || cert.organization,
            cert.year || cert.date || cert.issuedAt,
          ].filter(Boolean).join(' - ');

      if (ss(label)) certifications.add(ss(label));
    });
    ...
    if (resume?.content) {
      try {
        ...
        sa(resumeData.education).forEach((edu) => {
          if (!edu) return;
        ...
        sa(resumeData.certifications).forEach((cert) => {
          if (!cert) return;
        ...
        // malformed resume content — skip silently
      }
    }

    const educationArr = Array.from(education).filter(Boolean).slice(0, 8);
    const certificationArr = Array.from(certifications).filter(Boolean).slice(0, 8);

    return {
      education: educationArr,
      certifications: certificationArr,
      combined: Array.from(new Set([...educationArr, ...certificationArr])).slice(0, 12),
    };
  } catch {
    return {
      education: [],
...
// ─── Mode filter ──────────────────────────────────────────────────────────────
// Applies mode-based field selection to a full promptContext object.
// Always includes user + sourceStatus — they are metadata, not content.
// Returns a subset of the full context shaped for the requesting tool's needs.
// Unknown or missing mode → returns full context unchanged.
function applyMode(full, mode) {
  try {
    if (!mode || typeof mode !== 'string') return full;
    ...
    switch (mode.toLowerCase().trim()) {
      ...
      // Resume vs JD alignment — Signal Validation Engine
      // Needs: what they can prove, what's missing, caution flags
      case 'alignment':
        return {
          ...base,
          proofSignals: full.proofSignals,
          knownGaps:    full.knownGaps,
          cautionFlags: full.cautionFlags,
        };

      // Action-oriented tools — Promotion Engine, Growth Engine
      // Needs: what they've done, recent tool context, where they're positioned
      case 'execution':
        return {
          ...base,
          positioning:        full.positioning,
          proofSignals:       full.proofSignals,
          relevantWins:       full.relevantWins,
          knownGaps:          full.knownGaps,
          recentToolInsights: full.recentToolInsights,
        };

      // Career direction tools — Strategy, Roadmap, Growth
      // Needs: where they're headed, what's blocking them, latest insights
      case 'strategy':
        return {
          ...base,
          positioning:        full.positioning,
      ...
      // Offer & compensation tools — Negotiation Engine
      // Needs: identity, positioning, proof of value, latest negotiation context
      case 'negotiation':
        return {
          ...base,
          positioning:        full.positioning,
      ...
      // Recruiter-facing tools — Why Engine, Recruiter Mirror
      // Needs: who this person is, what they've proven, what they've done
      case 'recruiter':
        return {
          ...base,
          positioning:  full.positioning,
      ...
      // Coach-facing tools — Coaching Intelligence, Client Hub
      // Needs: where the gaps are, what to watch for, recent tool signals, positioning
      case 'coach':
        return {
          ...base,
          positioning:        full.positioning,
      ...
      // Signal matching — job matching, JD scoring
      // Needs: proof signals, positioning, data availability
      case 'matching':
        return {
          ...base,
          positioning:  full.positioning,
      ...
      // Growth & Pivot — fullest non-default mode
      // Needs: complete picture for pivot reasoning — what's proven, what transfers,
      // what gaps constrain the path, what flags should prevent unsupported recommendations
      case 'growth':
        return {
          ...base,
          positioning:        full.positioning,
      ...
      // Unknown mode — return full context, never fail
      default:
        return full;
    }
  } catch (err) {
    console.error('[buildPromptContext] applyMode error:', err?.message || err);
    return full; // always fall back to full context
  }
}

// ─── Main export ───────────────────────────────────────────────────────────────
export async function buildPromptContext({ userId, mode } = {}) {
  ...
  try {
	...
	const credentials = extractCredentials(profile, resume);
