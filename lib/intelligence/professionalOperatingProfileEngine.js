...
function buildPersonProfile(answers = {}, evidenceSignals = []) {
  ...
  out.push(
    a.motivation
      ? `This person appears primarily driven by ${optionLabel(a.motivation)}.`
      : 'This person’s motivation pattern should be refined through additional reflection.'
  );

  if (a.careerHope) {
    out.push(`They are seeking more ${optionLabel(a.careerHope, a.careerHope)} from their career.`);
  }

  if (a.idealImpact) {
    out.push(`They want their work to create this kind of impact: ${a.idealImpact}`);
  }

  if (hasSignal(evidenceSignals, /mission|ownership|impact|leadership|service/i)) {
    out.push('Evidence suggests they care about useful work, accountability, and whether effort translates into visible value for others.');
  }
...
function buildConfidenceSignals(reflection = [], evidenceSignals = []) {
  ...
  for (const item of combined) {
    ...
    const buckets = [
      ...
      ['strategic thinking', /strategic|strategy|business|platform|product/i],
      ['problem solving', /problem|solver|improved|analysis|analytical/i],
      ['pressure stability', /calm|pressure|stabiliz|risk/i],
      ['learning / adaptability', /learner|ambiguity|adaptability|hands-on|iterative/i],
    ];
...
export function buildProfessionalOperatingProfile({ answersJson = {}, identityContext = {} } = {}) {
  ...
  const topIdentitySignals = unique(
    [
  ...
  const personProfile = buildPersonProfile(answers, topIdentitySignals);
  const professionalProfile = buildProfessionalProfile(operatingStyle, answers, topIdentitySignals);
