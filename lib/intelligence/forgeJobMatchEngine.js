...
function scoreJobAlignment(job, seeker = {}) {
  ...
  if (roleHits > 0) {
    const points = Math.min(28, 12 + roleHits * 4);
  ...
  if (primaryCluster) {
    ...
    const points = Math.min(30, 10 + sharedStrength * 4);

    score += points;

    reasons.push(`${primaryCluster.key} capability ecosystem aligns.`);
    ...
    evidence.push({
      ...
      text:
        primaryCluster.exactOverlap.length > 0
          ...
          : `Your signals: ${primaryCluster.seekerHits.slice(0, 6).join(", ")} | Job signals: ${primaryCluster.jobHits.slice(0, 6).join(", ")}`,
      points,
    });
  }
  ...
  if (!matchedSkills.length && !primaryCluster && !roleHits) {
    gaps.push("No strong profile or primary resume evidence was detected for this role.");
  }
