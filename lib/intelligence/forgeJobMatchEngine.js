...
function scoreJob(job, filters = {}) {
  ...
  const searchIntent = getJobSearchIntent(intent.role || intent.keyword);
const intentClusterKeys = [
  ...(searchIntent.primaryClusters || []),
  ...(searchIntent.adjacentClusters || []),
];

const primaryIntentKey = searchIntent.intentKey;
const primaryConflicts =
  primaryIntentKey && CONFLICTING_ECOSYSTEMS[primaryIntentKey]
    ? CONFLICTING_ECOSYSTEMS[primaryIntentKey]
    : [];

const titleConflictHits = primaryConflicts.filter((term) =>
  includesAny(j.title, [term])
);

if (primaryIntentKey && titleConflictHits.length > 0) {
  return {
    job: job || {},
    score: 0,
    tier: "No Signal",
    reasons: [],
    evidence: [],
    gaps: [`Excluded because the job title conflicts with "${intent.keyword || intent.role}".`],
  };
}

  const exactKeywordTitleHit =
    intent.keyword && lower(j.title).includes(lower(intent.keyword));

  const keywordTokenTitleHits = intent.keyword
    ? unique(intent.keyword.split(/\s+/g).filter(Boolean)).filter((term) =>
        includesAny(j.title, [term])
      ).length
    : 0;
  ...
  if (intent.roleTerms.length) {
    ...
    if (titleHits > 0 || bodyHits > 0) {
      const points = titleHits > 0
        ? exactKeywordTitleHit
          ? Math.min(62, 48 + titleHits * 6)
          : Math.min(52, 30 + titleHits * 8)
        : Math.min(22, bodyHits * 6);
...
const clusterResults = clusterEntries
  ...
  if (exactKeywordTitleHit) {
    const titleFloor = countMatches(j.description, intent.keywordTerms) > 0 ? 90 : 86;
    score = Math.max(score, titleFloor);

    reasons.push("Exact search phrase appears in the job title.");
    evidence.push({
      label: "Exact title match",
      text: j.title,
      points: titleFloor,
    });
  } else if (intent.keyword && keywordTokenTitleHits >= 2) {
    score = Math.max(score, 72);

    reasons.push("Multiple search terms appear in the job title.");
    evidence.push({
