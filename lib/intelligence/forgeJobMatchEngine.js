...
function scoreJobAlignment(job, seeker = {}) {
  ...
  const roleHits = countMatches(
    searchableJobText,
    expandTerms([seekerHeadline])
  );

  if (roleHits > 0) {
    const points = Math.min(35, 18 + roleHits * 6);

    score += points;

    reasons.push("Career trajectory aligns with this role.");
  ...
  if (matchedSkills.length > 0) {
    const points = Math.min(40, matchedSkills.length * 4);

    score += points;

    reasons.push("Skills and experience align with the job.");

    evidence.push({
      label: "Skill alignment",
      text: matchedSkills.slice(0, 10).join(", "),
      points,
    });
  }
  ...
  const clusterResults = Object.entries(CAPABILITY_CLUSTERS)
    .map(([key, terms]) => {
      ...
      return {
        key,
        overlap: seekerHits.filter((term) =>
          jobHits.includes(term)
        ),
      };
    })
  ...
  if (primaryCluster) {
    const points = Math.min(30, primaryCluster.overlap.length * 5);

    score += points;

    reasons.push(
      `${primaryCluster.key} ecosystem strongly aligns.`
    );

    evidence.push({
      label: "Capability ecosystem",
      text: primaryCluster.overlap.join(", "),
      points,
    });
  }
  ...
  if (
    ...
    includesAny(j.location, preferredLocations)
  ) {
    score += 8;
