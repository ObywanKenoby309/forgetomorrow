...
function recruiterInterpretationForSignal(sig, profileData = {}, jobContext = null) {
  ...
  if (status === 'direct') {
    switch (sig.key) {
      case 'credentials': {
        const vectors = credibilityVectors(profileData);
        return `Credibility evidence is present through ${vectors.length ? vectors.join(", ") : "multiple professional proof vectors"}.`;
      }
      case 'portfolio':
        return 'Portfolio evidence supports execution credibility.';
      case 'visibility':
        return 'Portfolio and resume visibility support recruiter verification.';
      default:
        return `${sig.label} is clearly defined and recruiter-visible.`;
    }
  }
  ...
  switch (sig.key) {
    case 'identity':
      return status === 'adjacent'
        ? 'ForgeTomorrow sees a basic professional identity signal, but the headline could better clarify role, level, and direction. Role direction could be clearer.'
    ...
    case 'narrative':
      return status === 'adjacent'
        ? 'ForgeTomorrow sees a usable professional narrative, but it needs sharper value, direction, and proof. Positioning needs sharper direction and proof.'
        : 'ForgeTomorrow does not yet see enough narrative context. The portfolio lacks enough narrative context.';
