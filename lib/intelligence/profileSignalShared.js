...
export function signalScoreVsJD(profileData, jobContext) {
  ...
  // Count how many signals are relevant to the JD context
  const jdText = String(
    jobContext?.jobDescription || jobContext?.description || ""
  ).toLowerCase();
  const jobTitle = String(
    jobContext?.title || jobContext?.jobTitle || ""
  ).toLowerCase();
  const hasJDContext = Boolean(jdText || jobTitle);
  return {
    score,
    label: verdict.label,
