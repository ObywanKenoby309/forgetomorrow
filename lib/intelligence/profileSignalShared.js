...
// - Return structured signal evidence for WHY/profile rendering:
//   evidenceDetected, missingValidation, recruiterRisk, confidenceLevel, evidenceSummary, and seekerCoaching
...
function recruiterInterpretationForSignal(sig, profileData = {}, jobContext = null) {
  ...
  if (status === 'direct') {
    switch (sig.key) {
      case 'credentials': {
        const vectors = credibilityVectors(profileData);
        return `ForgeTomorrow identified ${vectors.length ? vectors.join(', ') : 'multiple credibility vectors'} as practical credibility evidence. This signal reflects execution, leadership, training, service background, portfolio proof, certifications, and other professional trust indicators beyond formal academic pedigree alone.`;
      }
      case 'portfolio':
        return 'Portfolio evidence demonstrates visible project ownership, execution scope, and measurable work beyond standard resume claims.';
      case 'visibility':
        return "Primary resume and profile visibility settings improve recruiter access, validation, and follow-through.";
      default:
        return `${sig.label} evidence is present${hasJobContext ? " for this role context" : ""}.`;
    }
  }
  ...
  switch (sig.key) {
    ...
    case 'availability':
      {
      const wp = profileData.workPreferences || {};
      const workType = wp.workType || wp.preferredWorkType || 'Not specified';
      const schedule = wp.schedule || 'Not specified';

      if (status === 'adjacent') {
        return `Availability Signal shows partial recruiter logistics context. Work type: ${workType}. Schedule: ${schedule}. Additional recruiter confirmation may still be needed.`;
      }

      return 'Availability details are limited or missing. Recruiters may need to confirm work type, scheduling expectations, location flexibility, or start timing directly with the candidate.';
    }

    case 'language':
      {
      const languages = safeArr(profileData.languages)
        .map((l) => labelFromItem(l))
        .filter(Boolean);

      if (!languages.length) {
        return 'No languages were listed. ForgeTomorrow treats language as informational unless the target role explicitly requires language capability.';
      }

      return `Candidate lists ${languages.join(', ')} as communication language evidence. This signal should only materially affect evaluation if the role requires language alignment.`;
    }
...
function signalImpactForSignal(sig) {
  switch (sig.key) {
    ...
    case 'narrative':
      return 'Clearer narrative positioning improves recruiter understanding of professional direction, value, and role alignment.';
    case 'proof':
      return 'Structured skill evidence improves recruiter search visibility and reduces guesswork during screening.';
    case 'portfolio':
    ...
    case 'availability':
      return 'Availability details help recruiters quickly determine whether work structure, scheduling, and hiring logistics are realistically aligned.';
    case 'language':
      return 'Language details provide communication context for roles requiring multilingual, international, remote, or customer-facing collaboration.';
    case 'visibility':
      return 'Resume access and profile visibility directly affect discoverability, recruiter validation speed, and follow-through.';
    default:
