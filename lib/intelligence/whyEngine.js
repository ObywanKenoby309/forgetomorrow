...
function titleTokens(value = "") {
  const stop = new Set([
    "the", "and", "or", "of", "for", "to", "in", "with", "a", "an",
    "role", "position", "job", "full", "time", "remote", "hybrid", "onsite", "on", "site",
  ]);
...
function extractLikelyJobTitle(jobDescription = "") {
  const raw = String(jobDescription || "").replace(/\r/g, "\n");
  const lines = raw
    .split(/\n+/g)
  ...
  const stopMarkers = [
    " location", " employment type", " about the role", " responsibilities",
    " what you'll do", " what you will do", " required", " requirements",
    " qualifications", " success in this role",
  ];

  const firstUsefulLine = lines.find((line) => {
    const lower = line.toLowerCase();
    if (lower.length < 4 || lower.length > 120) return false;
    return !stopMarkers.some((marker) => lower === marker.trim());
  }) || "";

  let candidate = firstUsefulLine;
  const lowerCandidate = ` ${candidate.toLowerCase()} `;

  for (const marker of stopMarkers) {
    const idx = lowerCandidate.indexOf(marker);
    if (idx > 0) {
      candidate = candidate.slice(0, Math.max(0, idx - 1)).trim();
      break;
    }
  }
...
function titleOverlapScore(sourceTitle = "", compareText = "") {
  const sourceTokens = Array.from(new Set(titleTokens(sourceTitle)));
  if (!sourceTokens.length) return 0;

  const compare = ` ${normalizeTitleText(compareText)} `;
  const hits = sourceTokens.filter((token) => compare.includes(` ${token} `)).length;
  return hits / sourceTokens.length;
}

function detectDirectRoleHistory(resumeText, jobDescription) {
  const resume = String(resumeText || "");
  const resumeLower = normalizeTitleText(resume);
  const jdTitle = extractLikelyJobTitle(jobDescription);
  const jdTitleTokens = titleTokens(jdTitle);

  if (!jdTitle || jdTitleTokens.length < 2) {
    return {
  ...
  const exactTitlePhraseMatch = resumeLower.includes(jdTitle);
  ...
  const bestTitleOverlap = resumeLines.reduce((best, line) => {
    return Math.max(best, titleOverlapScore(jdTitle, line));
  }, titleOverlapScore(jdTitle, resumeLower));

  const directRoleMatch = exactTitlePhraseMatch || (jdTitleTokens.length >= 3 && bestTitleOverlap >= 0.75);
...
export function buildExplain(resumeText, jobDescription) {
  ...
  const roleHistory = detectDirectRoleHistory(resume, jd);
...
const matchedRequiredSignals = matchedSignals.filter((match) =>
  ...
  const roleAdjustedScore = roleHistory.directRoleMatch
    ? Math.min(100, cov.score + 12)
    : cov.score;

  let grade = gradeFrom(roleAdjustedScore, cov.tierAHit, cov.tierARequiredForStrong);

  if (roleHistory.directRoleMatch && grade !== "Strong") {
    grade = "Strong";
  }
  ...
  return {
    ...
    _debug: {
      roleHistory,
      roleAdjustedScore,
      originalCapabilityScore: cov.score,
    ...
    match: { score: roleAdjustedScore, grade, summary, disclaimer },
    signals: {
...
// ─── Profile signal explainability ───────────────────────────────────────────
// This extends WHY beyond resume-vs-JD matching without creating a second brain.
// profileSignalShared classifies and extracts structured evidence; WHY converts
// that evidence into a presentation-safe, evidence-first interpretation object.

function cleanEvidenceLabel(text) {
...
function profileEvidenceList(signal, max = 4) {
  const raw = Array.isArray(signal?.evidenceDetected) ? signal.evidenceDetected : [];
  const seen = new Set();
  const out = [];
  for (const item of raw) {
    const cleaned = cleanEvidenceLabel(item);
    const key = normalizeTerm(cleaned);
...
function lowerJobText(jobContext) {
  if (!jobContext) return '';
  if (typeof jobContext === 'string') return safeLower(jobContext);
  return safeLower([
    jobContext.title,
    jobContext.jobTitle,
    jobContext.company,
...
function detectWorkStructure(jobLower) {
  const found = [];
  if (/\bremote\b/.test(jobLower)) found.push('remote');
  if (/\bhybrid\b/.test(jobLower)) found.push('hybrid');
  if (/\bon[- ]?site\b|\bin office\b/.test(jobLower)) found.push('on-site');
  if (/\bfull[- ]?time\b/.test(jobLower)) found.push('full-time');
  if (/\bpart[- ]?time\b/.test(jobLower)) found.push('part-time');
  if (/\bcontract\b|\bcontractor\b/.test(jobLower)) found.push('contract');
  return Array.from(new Set(found));
}

function detectLanguageRequirements(jobLower) {
  const languages = [];
  const langTerms = ['spanish','french','german','mandarin','chinese','cantonese','japanese','korean','arabic','russian','portuguese','italian','latvian','english'];
  const requiresLanguage = /\b(bilingual|multilingual|fluent|native speaker|language requirement|must speak|proficiency in)\b/.test(jobLower);
  for (const lang of langTerms) {
    if (new RegExp(`\\b${lang}\\b`, 'i').test(jobLower)) languages.push(lang);
  }
  return { required: requiresLanguage || languages.length > 1, languages: Array.from(new Set(languages)) };
}

function buildProfileSignalInterpretation(signal, profileData = {}, jobContext = null) {
  const key = signal?.key;
  const status = signal?.status;
  const evidence = profileEvidenceList(signal, 4);
  const jobLower = lowerJobText(jobContext);
  const wp = profileData?.workPreferences || {};

  const base = {
    ...signal,
    interpretationTitle: signal?.label || 'Profile Signal',
    interpretationSummary: '',
    recruiterContext: evidence,
    recruiterFocus: '',
    signalGroup: ['availability', 'language', 'visibility'].includes(key) ? 'recruiter_readiness' : 'capability_evidence',
  };

  if (key === 'identity') {
    return {
      ...base,
      interpretationTitle: 'Professional Positioning',
      interpretationSummary: status === 'direct'
        ? 'Candidate identity and role positioning are clearly visible.'
        : 'Candidate identity is present but should be clarified for faster recruiter recognition.',
      recruiterFocus: status === 'direct' ? 'Use headline context to quickly understand positioning.' : 'Validate role target and seniority during review.',
    };
  }
  ...
  if (key === 'narrative') {
    return {
      ...base,
      interpretationTitle: 'Professional Narrative',
      interpretationSummary: status === 'direct'
        ? 'Profile narrative provides enough context to understand direction, ownership, and professional value.'
        : 'Profile narrative needs more role direction, outcome proof, or value clarity.',
      recruiterFocus: 'Use the summary to understand professional direction, then validate outcomes through resume/project evidence.',
    };
  }
  ...
  if (key === 'proof') {
    return {
      ...base,
      interpretationTitle: 'Capability Proof',
      interpretationSummary: status === 'direct'
        ? 'Structured skills are present and usable for search, screening, and capability validation.'
        : 'Capability evidence is present but not yet strong enough to carry search and screening weight on its own.',
      recruiterFocus: 'Review listed skills against role requirements and supporting resume evidence.',
    };
  }
  ...
  if (key === 'portfolio') {
    return {
      ...base,
      interpretationTitle: 'Portfolio Evidence',
      interpretationSummary: status === 'direct'
        ? 'Portfolio entries show visible project ownership, execution scope, and outcome-oriented proof.'
        : status === 'adjacent'
        ? 'Portfolio work is present, but measurable outcomes or ownership details need stronger validation.'
        : 'No portfolio/project evidence is available for recruiter validation.',
      recruiterFocus: 'Use project entries to validate execution beyond resume claims.',
    };
  }

  if (key === 'credentials') {
    const vectors = evidence.filter((x) => !/^Education:/i.test(x) && !/^Certification\/training:/i.test(x));
    return {
      ...base,
      interpretationTitle: 'Professional Credibility',
      interpretationSummary: vectors.length
        ? `Credibility evidence is supported by ${vectors.join(', ')}.`
        : 'Structured credibility evidence is limited or not yet visible.',
      recruiterFocus: 'Review credibility through execution, leadership, training, service, certifications, portfolio proof, and measurable outcomes — not academic pedigree alone.',
    };
  }

  if (key === 'availability') {
    const workStructure = detectWorkStructure(jobLower);
    const candidateWorkType = wp.workType || wp.preferredWorkType || null;
    const candidateSchedule = wp.schedule || null;
    const context = [];
    if (wp.workStatus || wp.status || wp.openTo) context.push(`Work status: ${wp.workStatus || wp.status || wp.openTo}`);
    if (candidateWorkType) context.push(`Work type: ${candidateWorkType}`);
    if (candidateSchedule) context.push(`Schedule: ${candidateSchedule}`);
    if (wp.startDate || wp.earliestStartDate) context.push(`Start timing: ${wp.startDate || wp.earliestStartDate}`);

    let summary = 'Candidate availability information is provided for recruiter review.';
    let focus = 'Confirm availability only if work structure, schedule, location, or start timing is material to the role.';
    if (workStructure.length) {
      const candidateCombined = `${candidateWorkType || ''} ${candidateSchedule || ''}`.toLowerCase();
      const aligned = workStructure.some((term) => candidateCombined.includes(term));
      summary = aligned
        ? `Candidate availability appears aligned with the role’s stated work structure (${workStructure.join(', ')}).`
        : `Role mentions work structure (${workStructure.join(', ')}); candidate availability should be confirmed against that requirement.`;
      focus = aligned ? 'No obvious work-structure conflict detected from provided data.' : 'Validate work type, schedule, and location fit before advancing.';
    } else if (jobLower) {
      summary = 'No explicit work-structure requirement was detected in the job description; candidate availability is shown for context.';
    }

    return { ...base, interpretationTitle: 'Recruiter Logistics Context', interpretationSummary: summary, recruiterContext: context.length ? context : evidence, recruiterFocus: focus };
  }

  if (key === 'language') {
    const langReq = detectLanguageRequirements(jobLower);
    const langs = evidence.length ? evidence : [];
    let summary = 'Language information is provided for recruiter context only.';
    let focus = 'Do not treat language as a scoring factor unless the role explicitly requires it.';
    if (langReq.required) {
      summary = `Role appears to include language requirements${langReq.languages.length ? ` (${langReq.languages.join(', ')})` : ''}; compare against candidate language evidence.`;
      focus = 'Validate required language capability if it is material to the role.';
    }
    return { ...base, interpretationTitle: 'Language Context', interpretationSummary: summary, recruiterContext: langs, recruiterFocus: focus };
  }

  if (key === 'visibility') {
    return {
      ...base,
      interpretationTitle: 'Recruiter Access & Verification',
      interpretationSummary: status === 'direct'
        ? 'Primary resume and profile visibility are available for recruiter access and validation.'
        : 'Recruiter access or verification data is incomplete.',
      recruiterFocus: 'This is an accessibility/readiness signal, not a candidate capability score.',
    };
  }
...
export function buildProfileExplain(profileData = {}, jobContext = null) {
  const signals = classifySignals(profileData, jobContext || null).map((signal) =>
    buildProfileSignalInterpretation(signal, profileData, jobContext)
  );
  const verdict = overallVerdict(signals);
  const score = signalScoreToPercent(verdict);
  const capabilitySignals = signals.filter((s) => s.signalGroup !== 'recruiter_readiness');
  const readinessSignals = signals.filter((s) => s.signalGroup === 'recruiter_readiness');

  return {
    score,
    verdict,
    summary: `${verdict.proven} proven, ${verdict.partial} partial, ${verdict.missing} missing profile signals.`,
    signals,
    capabilitySignals,
    readinessSignals,
    demonstrated: signals.filter((s) => s.status === 'direct').map((s) => s.label),
    validation: signals.filter((s) => s.status !== 'direct').map((s) => s.label),
  };
}
