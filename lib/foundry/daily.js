...
export async function createDailyRoom(roomId) {
  const res = await fetch(`${DAILY_API}/rooms`, {
    ...
    headers: headers(),
    body: JSON.stringify({
      name: roomId,                        // matches your Foundry roomId
      ...
      properties: {
        ...
        exp: Math.floor(Date.now() / 1000) + 60 * 60 * 8, // 8hr expiry
...
// isOwner = true for the host (gives them meeting controls).
export async function createDailyToken({ roomId, userId, userName, isOwner = false }) {
  const res = await fetch(`${DAILY_API}/meeting-tokens`, {
    ...
    body: JSON.stringify({
      properties: {
        ...
        start_audio_off: true,            // muted by default, matches your UI
      },
    }),
  });
