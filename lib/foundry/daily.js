...
export async function createDailyToken({ roomId, userId, userName, isOwner = false, avatarUrl = null, role = null }) {
  ...
  // IMPORTANT:
  // Daily meeting-token properties are strict. Do not send custom fields like
  // user_data here; Daily rejects unsupported token properties with
  // invalid-request-error. Participant metadata such as avatarUrl/role/userId
  // must be handled client-side with setUserData() after join, or through
  // app-level participant mapping.
  const properties = {
    room_name: roomId,
    user_id: safeUserId,
