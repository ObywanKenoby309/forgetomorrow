...
export async function createDailyToken({ roomId, userId, userName, isOwner = false, avatarUrl = null, role = null }) {
  ...
  // userData is readable by all participants via p.userData — used for:
  // - avatarUrl (avatar display when camera is off)
  // - role (co-host eligibility filtering)
  // - userId (targeting controls at specific users)
  const userData = {
    userId: safeUserId,
    ...(avatarUrl ? { avatarUrl } : {}),
    ...(role ? { role } : {}),
  ...
  const res = await fetch(`${DAILY_API}/meeting-tokens`, {
    ...
    body: JSON.stringify({
      properties: {
        ...
        exp: Math.floor(Date.now() / 1000) + 60 * 60 * 8,
        enable_screenshare: true,
        start_video_off: true,   // always start video off — user toggles on
        start_audio_off: true,
        // participant_admin gives owner ability to mute/stop-camera remotes
        ...(isOwner ? { enable_prejoin_ui: false } : {}),
