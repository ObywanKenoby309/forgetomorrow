...
// Title equivalency map -- direct promotion.
// When a resume contains any of these phrases for a given signal, the candidate
// has held a role that IS that capability cluster. Do not require pattern counting.
// Responsibility-first: the title proves the function was performed.
const titleEquivalencyMap = {
  'advisory and client service delivery': [
    'customer success', 'client success', 'client services', 'service delivery',
    'account operations', 'customer operations', 'client operations',
    'account management', 'client management', 'success operations',
    'account team lead', 'client team lead', 'service team lead',
    'enterprise accounts', 'strategic accounts', 'managed accounts',
    'customer experience', 'client experience', 'customer engagement',
  ],
  'people leadership and team management': [
    'team lead', 'team leader', 'team manager', 'people manager',
    'operations manager', 'operations lead', 'delivery manager', 'delivery lead',
    'managed a team of', 'led a team of', 'supervised a team',
    'direct reports', 'hybrid team', 'offshore team', 'distributed team',
  ],
  'delivery and execution': [
    'operations lead', 'delivery lead', 'implementation lead', 'program lead',
    'program manager', 'delivery manager', 'implementation manager',
    'site lead', 'site manager', 'account lead', 'account manager',
    'sla', 'service level agreement', 'kpi', 'escalation management',
    'operational performance', 'operational outcomes', 'service performance',
  ],
  'ownership and accountability': [
    'account team lead', 'operational team lead', 'service team lead',
    'director of', 'head of', 'vp of', 'vice president of',
    'chief', 'founder', 'ceo', 'coo', 'owner',
    'portfolio management', 'portfolio ownership', 'account portfolio',
  ],
  'stakeholder and executive engagement': [
    'executive reporting', 'executive communication', 'c-suite',
    'leadership reporting', 'cross-functional leadership', 'enterprise client',
    'strategic account', 'executive sponsor', 'senior stakeholder',
  ],
  'process and methodology development': [
    'operations manual', 'playbook', 'standard operating procedure', 'sop',
    'onboarding program', 'training program', 'operational framework',
    'process improvement', 'operational excellence', 'continuous improvement',
    'workflow design', 'workflow optimization', 'process engineering',
  ],
};

function checkTitleEquivalency(signal, source) {
  const normalizedSignal = normalize(signal);
  const equivalents = titleEquivalencyMap[normalizedSignal];
  if (!equivalents || !equivalents.length) return false;
  return equivalents.some((phrase) => source.includes(normalize(phrase)));
}
...
export function evaluateSignal(signal, resume = {}) {
  ...
  // Title equivalency check -- runs before pattern counting.
  // If the resume contains a known equivalent title/function phrase for this signal,
  // the candidate has performed this capability directly. Promote to 'direct'
  // regardless of whether the exact signal phrase appears verbatim.
  if (checkTitleEquivalency(signal, source)) {
    return {
      signal,
      status: 'direct',
      confidence: 'high',
      evidence: `Direct evidence: resume demonstrates a role or function equivalent to "${signal}".`,
    };
  }
  ...
  if (matched.length) {
    ...
    // Responsibility-match promotion:
    // When a resume saturates a signal with 6+ matched patterns, or 4+ domain-relevant
    // matches, the candidate has DEMONSTRATED the responsibility directly through their
    // bullets and history. The title differing from the JD title is irrelevant at this
    // point. Promote to 'direct' so the AI doesn't hedge on proven capability.
    const isDirect = matched.length >= 6 || technicalMatches.length >= 4;
    ...
    const confidence = isDirect || technicalMatches.length >= 2 || matched.length >= 3
      ? 'high'
      : 'medium';
    ...
    return {
      ...
      confidence,
      evidence: isDirect
        ? `Direct evidence: responsibilities proven through ${matched.slice(0, 6).join(', ')}.`
