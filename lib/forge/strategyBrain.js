...
export function normalizeResumeForBrain(resume = {}) {
  ...
  return {
    ...
    experiences: normalizeList(experiences),
    projects: normalizeList(resume.projects),
    volunteerExperiences: normalizeList(resume.volunteerExperiences),
    education: normalizeList(education),
    certifications: normalizeList(resume.certifications),
    languages: normalizeList(resume.languages),
    achievements: normalizeList(resume.achievements),
    customSections: normalizeList(resume.customSections),
  };
}
...
export function buildResumeSnapshot(resume = {}, limits = {}) {
  ...
  const educationText = normalized.education
    .slice(0, 4)
    .map((ed, index) => {
      if (typeof ed === 'string') return `${index + 1}. ${ed}`;
      const school = ed.school || ed.institution || ed.name || '';
      const degree = ed.degree || '';
      const field = ed.field || ed.program || ed.major || '';
      const notes = ed.notes || ed.description || '';
      return `${index + 1}. ${[degree, field, school, notes].filter(Boolean).join(' | ')}`;
    })
    .join('\n');
  ...
  const languageText = normalized.languages
    .slice(0, 8)
    .map((lang, index) => `${index + 1}. ${typeof lang === 'string' ? lang : [lang.name || lang.language || lang.label, lang.proficiency || lang.level].filter(Boolean).join(' | ')}`)
    .join('\n');

  const achievementText = normalized.achievements
    .slice(0, 8)
    .map((achievement, index) => `${index + 1}. ${typeof achievement === 'string' ? achievement : [achievement.title || achievement.name, achievement.description || achievement.summary].filter(Boolean).join(' | ')}`)
    .join('\n');

  const customSectionText = normalized.customSections
    .slice(0, 4)
    .map((section, index) => `${index + 1}. ${stringifyValue(section)}`)
    .join('\n');
  ...
  return {
    ...
    text: [
      ...
      '',
      'Custom Sections:',
      customSectionText || '[No custom sections provided]',
    ].join('\n'),
  };
}
...
${isOverview ? `{
  ...
  {"section": "education", "requiredSignal": "Education and credential credibility for this role", "resumeEvidence": "Read what education, certifications, or training the resume actually shows. State it specifically.", "hiringImpact": "Does the visible education strengthen recruiter confidence for this specific role? State the credibility impact clearly.", "ifTrue": "If relevant education exists, name it and explain why it strengthens credibility for this role.", "ifNotTrue": "If no relevant education exists, state that education is not a credibility factor for this role.", "futurePositioning": "How a recruiter interprets this candidate's educational background for this specific role."}
],
