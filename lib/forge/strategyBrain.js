...
// jdCount: count distinct matching terms from a list in jdText.
// This is how we produce defensible, JD-derived signal weights.
// The weight of each signal is proportional to how much the JD emphasizes it.
function jdCount(text, terms) {
  const source = normalizeText(text);
  return terms.filter((term) => source.includes(String(term || '').toLowerCase())).length;
}

// deriveHammerSignalWeights:
// Returns an array of { signal, termCount, weight } objects.
// weight is a percentage (0-100) that sums to 100 across all signals.
// Weights are derived from THIS JD's term frequency -- no hardcoding.
//
// Weighting logic (fully explainable):
// - Count how many distinct terms from each signal's term list appear in the JD
// - Signals with 0 matches are excluded (not required by this JD)
// - Education is always included at a minimum floor weight (8%) as a credibility signal
// - Required signals share 92% of total weight, proportional to their term counts
// - Education takes the remaining 8%
// - This means a JD that heavily emphasizes delivery will weight delivery heavily.
//   A JD that barely mentions stakeholders will weight that signal lightly.
//   The numbers come directly from what the employer wrote.
export function deriveHammerSignalWeights({ jdText = '', jobMeta = null } = {}) {
  ...
  const SIGNAL_TERMS = [
    {
      ...
      terms: ['own', 'ownership', 'responsible for', 'accountable for', 'oversee', 'overseeing',
        'manage', 'managing', 'in charge of', 'lead the', 'leading the', 'head of',
        'primary contact', 'point of contact', 'drives', 'drive the'],
    },
    {
      signal: 'delivery and execution',
      terms: ['deliver', 'delivery', 'execute', 'execution', 'implement', 'implementation',
    ...
    {
      signal: 'people leadership and team management',
      terms: ['manage a team', 'managing a team', 'team of', 'direct reports', 'people manager',
    ...
    {
      signal: 'advisory and client service delivery',
      terms: ['advise', 'advisory', 'consult', 'consulting', 'consultant', 'trusted advisor',
    ...
    {
      signal: 'stakeholder and executive engagement',
      terms: ['stakeholder', 'executive', 'senior leadership', 'senior management', 'c-suite',
    ...
    {
      signal: 'process and methodology development',
      terms: ['methodology', 'methodologies', 'process', 'processes', 'procedure', 'procedures',
    ...
    {
      signal: 'domain knowledge and qualification',
      terms: ['experience in', 'background in', 'knowledge of', 'expertise in', 'familiar with',
  ...
  // Count term hits per signal
  const scored = SIGNAL_TERMS.map(({ signal, terms }) => ({
    signal,
    termCount: jdCount(combined, terms),
  })).filter(({ termCount }) => termCount > 0); // only include signals the JD actually requires

  const totalTerms = scored.reduce((sum, s) => sum + s.termCount, 0);

  // Required signals share 92% of weight, proportional to term counts.
  // Education gets a fixed 8% floor -- always included as a credibility signal.
  ...
  const weighted = scored.map(({ signal, termCount }) => ({
    signal,
    termCount,
    weight: totalTerms > 0 ? Math.round((termCount / totalTerms) * REQUIRED_POOL) : Math.round(REQUIRED_POOL / scored.length),
    required: true,
  }));

  // Always add education as a credibility signal at fixed floor weight
  weighted.push({
    signal: 'education and credential credibility',
    termCount: jdCount(combined, ['degree', 'bachelor', 'master', 'mba', 'phd', 'certified', 'certification', 'licensed', 'credential']),
  ...
  if (rawSum !== 100 && weighted.length > 0) {
    const diff = 100 - rawSum;
    // Apply the rounding correction to the highest-weight required signal
    const topRequired = weighted.filter(s => s.required).sort((a, b) => b.weight - a.weight)[0];
    if (topRequired) topRequired.weight += diff;
  }

  return weighted;
}

// deriveHammerSignals: backwards-compatible wrapper.
// Returns just the signal names for existing callers (buildDeterministicHammerAnalysis etc.)
function deriveHammerSignals({ jdText = '', missing = {}, jobMeta = null } = {}) {
  const weighted = deriveHammerSignalWeights({ jdText, jobMeta });
  return uniqueStrings(weighted.map(s => s.signal)).slice(0, 8);
}
