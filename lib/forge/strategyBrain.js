...
2. Explain whether the risk is fatal, moderate, survivable, or likely ignored.
...
For Skills actions, if the exact tool is not already visible, ifTrue must say where the proof should come from: the Skills list should name the tool only if the candidate has used it, and the Experience section must prove where it was applied.
...
Good:
"The operational leadership experience can support delivery ownership if measurable coordination and execution evidence is clarified."
...
${isOverview ? `JSON SHAPE FOR OVERVIEW:
{
  ...
  "environment": "${hammerContext.environment.label}",
"matchAssessment": "Write your private recruiter read here. Lead with the biggest blocker. Name the strongest visible support. Use specific evidence from the JD and resume. Do NOT repeat these instructions in the output. Do NOT use template phrasing.",
"signalGaps": [
