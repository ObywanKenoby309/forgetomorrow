...
function deriveHammerSignals({ jdText = '', missing = {}, jobMeta = null } = {}) {
  ...
  if (jdHas(combined, ['enterprise architecture', 'cloud platform', 'cloud platforms', 'cloud architecture', 'security standards', 'data engineering'])) {
  ...
  return uniqueStrings(signals).slice(0, 10);
}
...
- If any fatal risk exists:
  ...
  - Use "major screening risk" for roles like AI evaluation, consulting adjacent, or technical operator roles where adjacent technical proof may still be accepted.
...
- For AI evaluation, research, or assessment roles: adjacent technical operators with strong implementation proof are often competitive. Adjust severity accordingly.
`.trim();
}
...
Good ifTrue:
"Evidence would need to show ownership of platform systems, APIs, technical workflows, evaluation processes, or standards — specifically what technical responsibility existed and what decisions the candidate personally made."

Good ifTrue:
"Evidence would need to show where AI systems, platform workflows, integrations, or output-quality review processes were evaluated, operationalized, or improved — and what proof exists that the candidate owned the outcome."

Good ifTrue:
"Evidence would need to show cloud platform implementation ownership, API-driven architecture work, or technical evaluation workflows tied to production systems — direct implementation accountability, not general familiarity."
...
- include "education" if:
  - the JD explicitly requires a degree, certification, or license
...
That is the elite-tier recruiter phrasing. Use it.` : `SINGLE SECTION REQUIREMENTS:
...
Rotate proof-language naturally.
Do not repeatedly fallback to the exact same proof phrase across sections.
Prefer contextual specificity tied to the actual resume evidence.
...
${isOverview ? `{
  ...
  {"section": "skills", "requiredSignal": "Specific technical verification signal derived from the JD.", "resumeEvidence": "Skills evidence read.", "hiringImpact": "Severity classification plus screening consequence.", "ifTrue": "Observable tool/platform/workflow proof structure tied to visible resume evidence.", "ifNotTrue": "Do-not-claim warning plus named strongest truthful adjacent proof.", "futurePositioning": "Recruiter tooling interpretation behavior."},
  ...
  {"section": "education", "requiredSignal": "Relevant education or credential support", "resumeEvidence": "Education evidence read.", "hiringImpact": "Credibility impact, not a hard requirement unless the JD requires it.", "ifTrue": "Acknowledge relevant education, certifications, or training that strengthens recruiter confidence.", "ifNotTrue": "Leave empty if education is relevant. If not relevant, state that education is not a decision factor.", "futurePositioning": "How education affects recruiter confidence."}
],
