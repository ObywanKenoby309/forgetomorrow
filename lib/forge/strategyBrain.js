...
Describe what strong proof looks like for THIS specific requirement.

Do not repeat a generic structure.

Tailor the explanation to the signal:
- For project management → show ownership, coordination, and delivery
- For tools → show how the tool was applied to a real workflow and what changed
- For AI/LLM → show how the system was integrated and what problem it solved
- For stakeholders → show how alignment, communication, or decisions were handled

The explanation must reflect the nature of the requirement, not reuse a generic template.
...
- The JSON must use the improvementActions object shape exactly. Do not return improvementActions as plain strings.
- Every improvementAction must include exactly one section value: "summary", "skills", "experience", or "education".
- Choose the section where the user should make the actual resume change.
SUMMARY SECTION RULE:
If selected section is "summary", evaluate ONLY the user's current Summary against the JD.

Primary question:
"Would a senior HR recruiter read this summary and see enough alignment with this job description to support a hiring decision in favor of the candidate?"

You must return feedback whether the summary is strong OR weak.

If the Summary is well aligned:
- Say what is working.
- Explain which JD signals the Summary already supports.
- Explain why that helps the candidate.
- Suggest only small improvements if needed.
- Do not invent gaps just to give criticism.

If the Summary is weak or incomplete:
- Say what is missing or unclear.
- Explain why the missing signal creates hiring risk.
...
For summary output:
- Every improvementAction must use "section": "summary".
- Do not route summary feedback to "skills", "experience", or "education".
- The requiredSignal should describe the summary alignment issue or strength.
- The resumeEvidence field should explain what the current Summary already proves or does not prove.
- The hiringImpact field should explain how this affects the recruiter's first impression.
- The ifTrue field should explain how to strengthen the Summary if the candidate has the missing evidence.
- The ifNotTrue field should explain what not to claim and what honest adjacent positioning to use instead.

If the resume does not prove a JD requirement, do not write it as fact.
Use conditional guidance only where needed.
- Role positioning, headline, and overall narrative go to "summary".
- Tools, technologies, APIs, and hard skills go to "skills".
- Projects, ownership, leadership, stakeholder work, and outcomes go to "experience".
- Degrees, certifications, licenses, and formal credentials go to "education".
- Do not use "overview" as a section value.
...
${
  ...
  "improvementActions": [
  {
    ...
    "resumeEvidence": "",
    "hiringImpact": "",
    "ifTrue": "A strong example should show what was built, led, or implemented; the scope of the work; who was involved; how execution was coordinated; and what changed as a result, if known.",
    "ifNotTrue": "Do not claim this experience. Strengthen the closest honest adjacent evidence already visible in the resume.",
    "futurePositioning": ""
  }
],
  "prioritySection": "",
