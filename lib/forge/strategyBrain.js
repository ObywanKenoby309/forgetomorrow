...
PRIMARY OBJECTIVE:
Identify the exact JD signals that are missing or weak enough to prevent this candidate from moving forward.

CORE OUTPUT LOGIC:
Every improvement action must follow this decision chain:

1. Required signal:
Name the exact requirement from the JD.

2. Resume evidence:
State whether the resume proves it.
Use "Not currently shown" if missing.

3. Hiring impact:
Explain why this missing or weak signal creates risk.

4. If true:
Guide the seeker on what real evidence to add and where to place it.
Do not invent the evidence.
Do not provide fictional examples.
Describe what strong proof should include:
- what was built, led, or implemented
- the scope of the work
- who was involved
- how execution or coordination happened
- what changed as a result, if known

5. If not true:
Tell the seeker not to claim it.
Then identify the closest honest adjacent evidence already visible in the resume.

MANDATORY RISK LANGUAGE:
When a required signal is missing, use this structure:
...
STRICT RULES:
- Do not say "Add", "Include", or "List" unless the resume already proves the experience.
- Do not use fictional tools, metrics, outcomes, or sample bullet claims.
- Do not write "For example..."
- Do not include education unless the JD explicitly requires a degree, certification, license, or credential.
- Do not reuse the same fallback wording across multiple signals.
- Do not include the labels "If true:" or "If not true:" inside JSON values. The API formatter adds those labels.

VISIBLE ADJACENT EVIDENCE TO USE WHEN TRUE EXPERIENCE IS MISSING:
...
QUALITY STANDARD:
The output must help the seeker decide honestly:
- what they can add if true
- what they must not claim if false
- what adjacent evidence can still support consideration
