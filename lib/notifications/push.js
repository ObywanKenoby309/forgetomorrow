...
/**
 * Wraps an avatar URL through our circular-crop endpoint so push notifications
 * (rendered by the OS, not CSS) show an actual round image with transparent
 * corners instead of a raw square. Safe no-op if the avatar URL is missing.
 */
function toCircularIcon(avatarUrl) {
  if (!avatarUrl) return null;
  const base = getBaseUrl();
  return `${base}/api/push/avatar-circle?url=${encodeURIComponent(avatarUrl)}&size=192`;
}
...
export async function sendPushToUser({
  ...
  try {
    ...
    // Any real avatar (not the default app icon) is routed through the circular-crop
    // endpoint so it renders as an actual round image, not a raw square.
    ...
    if (!resolvedIcon && actorUserId) {
      try {
        ...
        isAvatar = !!resolvedIcon;
      } catch {
        // Non-fatal — fall through to default icon
      }
    }

    if (resolvedIcon && isAvatar) {
      resolvedIcon = toCircularIcon(resolvedIcon) || resolvedIcon;
    }
